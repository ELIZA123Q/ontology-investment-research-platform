...
_incremental_spec = importlib.util.spec_from_file_location(
    ...
    ROOT / "05_governance/03_校验/incremental_update.py",
)
...
def ontology_catalog() -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]], set[str], set[str]]:
    ...
    for name in ("semantic", "state_event", "evidence", "judgment", "scenario", "semiconductor_extension"):
        document = load(ROOT / f"01_semantic/01_ontology/models/{name}.yaml")
...
def validate_ontology_instances(structure: dict[str, Any], label: str) -> list[str]:
    ...
    expected_refs = {
        f"01_semantic/01_ontology/models/{name}.yaml"
        for name in ("semantic", "state_event", "evidence", "judgment", "scenario", "semiconductor_extension")
    }
...
def known_rule_ids() -> set[str]:
    output: set[str] = set()
    for path in (ROOT / "01_semantic/01_ontology/models").glob("*.yaml"):
        output.update((load(path).get("rules") or {}).keys())
    output.update((load(ROOT / "01_semantic/01_ontology/models/semiconductor_extension.yaml").get("evidence_constraints") or {}).keys())
    return output


def main(argv: list[str]) -> int:
    runs = tuple(Path(arg).resolve() for arg in argv) or discover_runs()
    contract = load(ROOT / "05_governance/02_合同/public_contract.yaml")
