#!/usr/bin/env python3
"""Forbid parallel ontology model trees outside 01_semantic/01_ontology.

Machine-readable ontology YAML truth lives only under
01_semantic/01_ontology/{models,model_registry.yaml,meta_schema.yaml}.
...
TRUTH_MODELS = ROOT / "01_semantic" / "01_ontology" / "models"
TRUTH_REGISTRY = ROOT / "01_semantic" / "01_ontology" / "model_registry.yaml"
TRUTH_META = ROOT / "01_semantic" / "01_ontology" / "meta_schema.yaml"
...
ALLOW_SCAN_SKIP = {
    ...
    "__pycache__",
    ".venv",
    "04_execution",  # run artifacts / packs may embed copies
    "05_governance/14_evals",  # eval fixtures
}
...
def main() -> int:
    ...
    if not TRUTH_REGISTRY.is_file():
        errors.append("missing 01_semantic/01_ontology/model_registry.yaml")
    if not TRUTH_META.is_file():
        errors.append("missing 01_semantic/01_ontology/meta_schema.yaml")
    ...
    for models_dir in ROOT.rglob("models"):
        ...
        try:
            ...
            continue
        if rel == "01_semantic/01_ontology/models":
            continue
        ...
        if parent_name in {"01_ontology", "ontology", "01_通用"} or "ontology" in rel.split("/"):
            ...
            if yamls:
                errors.append(
                    ...
                    f"({len(yamls)} yaml files); write only under 01_semantic/01_ontology/models"
                )
    ...
    print("  ontology models may only live under 01_semantic/01_ontology/models")
    return 0
