...
``07_runtime/src/runtime``；本模块只保留旧正式包仍需验证的三个纯函数。
"""
...
def _catalog() -> tuple[set[str], set[str], dict[str, dict[str, Any]]]:
    ...
    model_root = ROOT / "01_semantic/01_ontology/models"
    for filename in MODEL_FILES:
        document = yaml.safe_load((model_root / filename).read_text(encoding="utf-8")) or {}
