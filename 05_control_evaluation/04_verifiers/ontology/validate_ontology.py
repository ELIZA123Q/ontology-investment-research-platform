...
this module lives in governance because it enforces rules rather than defining them.
"""
...
OPERATIONAL_MODEL = MODEL_DIR / "operational.yaml"
...
if str(_VALIDATOR_DIR) not in _sys.path:
    _sys.path.insert(0, str(_VALIDATOR_DIR))
...
def load_yaml_allow_merge(path: Path) -> dict[str, Any]:
    """Load YAML that intentionally uses merge keys (<<), e.g. kinetics action defaults."""
...
def validate_bundle(
    ...
    def validate_attribute(path: str, attribute: Any) -> None:
        ...
        if attr_type == "object_ref":
            ...
            if target not in set(all_objects) | PSEUDO_TYPES | operational_object_names():
            ...
            if section == "relation_types":
                ...
                for side in ("source_types", "target_types"):
                    ...
                    for endpoint in endpoints:
                        if endpoint not in all_objects and endpoint not in operational_object_names() and endpoint not in PSEUDO_TYPES:
            ...
            if section in ("rules", "evidence_constraints"):
                ...
                for key in ("applies_to", "input_types", "output_types"):
                    ...
                    if not _list_of_nonempty_strings(refs, allow_empty=False):
                        ...
                        refs = []
                    for ref in refs:
                        if ref not in set(all_objects) | PSEUDO_TYPES | operational_object_names():
...
def load_default_bundle() -> tuple[dict[str, dict[str, Any]], dict[str, Any], dict[str, Any]]:
    ...
    return models, load_yaml(EXTENSION), load_yaml(ONTOLOGY_DIR / "meta_schema.yaml")


def operational_object_names() -> set[str]:
    if not OPERATIONAL_MODEL.exists():
        return set()
    doc = load_yaml(OPERATIONAL_MODEL)
    return set((doc.get("object_types") or {}).keys())


def validate_scenario_catalog(all_object_ids: set[str], scenario_contract: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if not SCENARIO_CATALOG.exists():
        errors.append("02_scenario_task/02_scenarios/types.yaml is missing")
        return errors
    catalog = load_yaml(SCENARIO_CATALOG)
    if catalog.get("enforcement") != "catalog_only":
        errors.append("scenario_types.enforcement must be catalog_only")

    profiles_doc: dict[str, Any] = {}
    if not RESEARCH_REQUIREMENT_PROFILES.exists():
        errors.append("01_semantic_knowledge/01_ontology/research_requirement_profiles.yaml is missing")
    else:
        profiles_doc = load_yaml(RESEARCH_REQUIREMENT_PROFILES)

    semantic_profiles = profiles_doc.get("semantic_profiles") or {}
    judgment_profiles = profiles_doc.get("judgment_profiles") or {}
    known = all_object_ids | operational_object_names() | PSEUDO_TYPES
    required_fields = set(scenario_contract.get("required") or [])

    for profile_id, profile in semantic_profiles.items():
        if not isinstance(profile, dict):
            errors.append(f"semantic_profile:{profile_id} must be object")
            continue
        for key in (
            ...
            if not _list_of_nonempty_strings(values, allow_empty=False):
                errors.append(f"semantic_profile:{profile_id} {key} must be non-empty string list")
                continue
            for ref in values:
                if ref not in known:
                    errors.append(f"semantic_profile:{profile_id} unresolved type {ref}")
        relation_values = profile.get("required_relation_types") or []
        if not _list_of_nonempty_strings(relation_values, allow_empty=False):
            errors.append(f"semantic_profile:{profile_id} required_relation_types must be non-empty string list")

    for profile_id, profile in judgment_profiles.items():
        if not isinstance(profile, dict):
            errors.append(f"judgment_profile:{profile_id} must be object")
            continue
        values = profile.get("required_judgment_types") or []
        if not _list_of_nonempty_strings(values, allow_empty=False):
            errors.append(f"judgment_profile:{profile_id} required_judgment_types must be non-empty string list")
            continue
        for ref in values:
            if ref not in known:
                errors.append(f"judgment_profile:{profile_id} unresolved type {ref}")

    for scenario_id, scenario in (catalog.get("scenario_types") or {}).items():
        if not isinstance(scenario, dict):
            errors.append(f"scenario_types:{scenario_id} must be object")
            continue
        missing_scenario = sorted(required_fields - set(scenario))
        if missing_scenario:
            errors.append(f"scenario_types:{scenario_id} missing {missing_scenario}")
        for key in (
            "scope_dimensions",
            "entry_conditions",
            ...
            if key in required_fields and not _list_of_nonempty_strings(scenario.get(key), allow_empty=False):
                errors.append(f"scenario_types:{scenario_id} {key} must be non-empty string list")
        for text_key in ("id", "label_zh", "definition"):
            if text_key in required_fields and not isinstance(scenario.get(text_key), str):
                errors.append(f"scenario_types:{scenario_id} {text_key} must be string")
        semantic_req = scenario.get("semantic_requirements") or {}
        judgment_req = scenario.get("judgment_requirements") or {}
        if not isinstance(semantic_req, dict) or not semantic_req.get("profile"):
            errors.append(f"scenario_types:{scenario_id} semantic_requirements.profile required")
        elif semantic_req.get("profile") not in semantic_profiles:
            errors.append(f"scenario_types:{scenario_id} unknown semantic profile {semantic_req.get('profile')}")
        if not isinstance(judgment_req, dict) or not judgment_req.get("profile"):
            errors.append(f"scenario_types:{scenario_id} judgment_requirements.profile required")
        elif judgment_req.get("profile") not in judgment_profiles:
            errors.append(f"scenario_types:{scenario_id} unknown judgment profile {judgment_req.get('profile')}")
        # Scenario types must not re-embed ontology type lists
        for banned in (
            "required_object_types",
            ...
            if banned in scenario:
                errors.append(f"scenario_types:{scenario_id} must not embed {banned}; use profiles")
    return errors
...
def validate_migration_ledger(
    ...
    formal_objects |= operational_object_names()
    if OPERATIONAL_MODEL.exists():
        formal_relations.update((load_yaml(OPERATIONAL_MODEL).get("relation_types") or {}).keys())
    ...
    if (ROOT / "04_context_state/04_workspace/01_正式样例").exists():
...
def validate_platform_and_kinetics() -> list[str]:
    """Platform registry wiring + kinetic contract checks (former validate_v4)."""
    errors: list[str] = []
    ...
    audit_paths = [(ONTOLOGY_DIR / str(item)).resolve() for item in registry.get("audit_models") or []]
    all_models = [load_yaml_allow_merge(path) for path in semantic_paths + operational_paths + audit_paths]
    ...
    if any("scenario.yaml" in str(path) for path in semantic_paths):
        errors.append("scenario catalog must not remain registered as a semantic model")
    if not any(path.name == "ontology_extension.yaml" for path in semantic_paths):
        errors.append("semiconductor ontology_extension.yaml must be registered under semantic_models")
    operational_objects = {key for path in operational_paths for key in (load_yaml_allow_merge(path).get("object_types") or {})}
    if "ActionExecution" in operational_objects:
        errors.append("ActionExecution must not remain in operational model; use audit schema")
    if "SourceSnapshot" in operational_objects:
        errors.append("SourceSnapshot must live in evidence model, not operational")
    if "ResearchScope" not in operational_objects:
    ...
    action_doc = load_yaml_allow_merge(ONTOLOGY_DIR / str(kinetic.get("actions")))
    function_doc = load_yaml_allow_merge(ONTOLOGY_DIR / str(kinetic.get("functions")))
    policy_doc = load_yaml_allow_merge(ONTOLOGY_DIR / str(kinetic.get("policies")))
    trigger_doc = load_yaml_allow_merge(ONTOLOGY_DIR / str(kinetic.get("triggers")))
    ...
    for trigger_id, trigger in triggers.items():
        action_ref = trigger.get("action_ref")
        if action_ref not in actions:
            errors.append(f"{trigger_id}: unresolved action_ref {trigger.get('action_ref')}")
            continue
        run_as = trigger.get("run_as")
        if run_as not in allowed_actors:
            errors.append(f"{trigger_id}: run_as {run_as} is not a registered actor")
        action_actors = set((actions[action_ref] or {}).get("allowed_actors") or [])
        if run_as not in action_actors:
            errors.append(f"{trigger_id}: run_as {run_as} is not allowed by {action_ref}")
...
def main() -> int:
    ...
    object_ids = set()
    for model in models.values():
        object_ids.update((model.get("object_types") or {}).keys())
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        print(f"ONTOLOGY_INVALID: {len(errors)} error(s)")
        return 1
        ...
        for section in ("rules", "evidence_constraints"):
            for rule in (schema.get(section) or {}).values():
                if isinstance(rule, dict) and rule.get("test_case_role") == "executable":
                    exec_count += 1
    print(
        "ONTOLOGY_VALID: semantic depth + platform/kinetics; "
        f"objects={object_count}, relations={relation_count}, executable_rules={exec_count}"
    )
    return 0
