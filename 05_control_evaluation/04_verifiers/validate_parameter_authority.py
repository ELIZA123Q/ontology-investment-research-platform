...
def _validate_domain_semantics(domain_doc: dict, graph: dict, errors: list[str]) -> None:
    ...
    registry = yaml.safe_load((ROOT / "03_agent_capability/02_skills/evidence_research/registry.yaml").read_text(encoding="utf-8")) or {}
...
_SCAN_ALLOWLIST_SUFFIXES = {
    "05_control_evaluation/04_verifiers/status_derivation.py",  # 仅从本体规则加载集合
    "05_control_evaluation/04_verifiers/validate_parameter_authority.py",
    "05_control_evaluation/04_verifiers/knowledge_graph/ontology_instance_graph.py",
}
...
def _scan_duplicate_authority(errors: list[str]) -> None:
    ...
    for root in scan_roots:
        ...
        for path in root.rglob("*"):
            ...
            if "/tests/" in f"/{relative}/" or relative.startswith("05_control_evaluation/04_verifiers/tests/"):
                continue
...
def main() -> int:
    ...
    # 任务视图是 Runtime 的临时投影；不再要求仓库保存固定阶段 YAML 模板或样例副本。
    ...
    # 冻结 reference runs 覆盖 03/04 的实例图形态。
    coverage["stage03_manifests"] = len(task_views)
    coverage["stage04_audits"] = len(task_views)
    raw_contract = yaml.safe_load((ROOT / "05_control_evaluation/01_rules/contracts/public_contract.yaml").read_text(encoding="utf-8"))
    for field in (
        "judgment_types",
        "judgment_levels",
    ...
    matrix_path = ROOT / "05_control_evaluation" / "01_rules" / "policies" / "parameter_authority_matrix.yaml"
    ...
    kb03 = yaml.safe_load((ROOT / "03_agent_capability/02_skills/evidence_research" / "registry.yaml").read_text(encoding="utf-8")) or {}
    if "ontology_authority_refs" not in kb03:
        errors.append("03_agent_capability/02_skills/evidence_research/registry.yaml 缺少 ontology_authority_refs")
    ...
    if coverage_rate < 1.0:
        ...
        return 1
    print(
        "PARAMETER_AUTHORITY_PASS: Runtime 投影、12 个证据画像、46 个状态变量、"
