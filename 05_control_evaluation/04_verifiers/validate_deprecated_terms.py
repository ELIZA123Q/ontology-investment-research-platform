...
策略来源：``docs/migrations/repository_history/ontology_evolution/2026-07-20_evidence_relation_rename.md``
与 ``docs/migrations/repository_history/ontology_evolution/2x_to_3_ledger.yaml`` 的 retirement_policy
（``legacy_publish_compatible: false``——旧字段不得进入正式产物）。

只检查 git 跟踪的文件（自动排除被 .gitignore 忽略的本地导出，
如 ``06_runtime/.data/local_runs/exports``），并跳过迁移证据目录
``docs/migrations/repository_history/ontology_evolution/``（该目录正当保留旧名作为迁移记录）。
"""
...
# 旧名 → 新名，镜像 docs/migrations/repository_history/ontology_evolution/2026-07-20_evidence_relation_rename.md。
...
# 正当保留旧名的路径前缀（迁移证据目录）。
ALLOWED_PREFIXES = ("docs/migrations/repository_history/ontology_evolution/",)
