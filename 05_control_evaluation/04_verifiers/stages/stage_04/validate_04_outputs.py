...
sys.path.insert(0, str(_ROOT / "05_control_evaluation" / "04_verifiers"))
sys.path.insert(0, str(_ROOT / "06_runtime" / "adapters"))
...
METHOD_LIBRARY_ROOT = _ROOT / "03_agent_capability/02_skills/judgment_reasoning"
...
def _load_method_catalog() -> dict[str, Path]:
    ...
    if not METHOD_LIBRARY_ROOT.is_dir():
        fail(f"缺少03_agent_capability/02_skills/judgment_reasoning目录: {METHOD_LIBRARY_ROOT}")
    ...
    if not catalog:
        fail("03_agent_capability/02_skills/judgment_reasoning未发现任何 A 方法卡")
    return catalog
...
def _validate_method_library_usage(audit: dict[str, object]) -> None:
    ...
    try:
        ...
        fail("method_library_usage.library_ref 必须落在03_agent_capability/02_skills/judgment_reasoning目录内")
    ...
    if unknown:
        fail("method_library_usage.methods_used 未在03_agent_capability/02_skills/judgment_reasoning注册: " + ", ".join(unknown))
