#!/usr/bin/env python3
"""业务本体实例图校验与 02 任务视图确定性投影。

Semantic 图合同在 ``01_semantic_knowledge/03_knowledge_graph/contracts/``。
本模块属于 Governance 校验执行层：校验实例图合法性，并在内存中恢复
阶段熟悉的业务视图；恢复结果是投影，不是第二权威源。
"""
...
sys.path.insert(0, str(ROOT / "05_control_evaluation" / "04_verifiers"))
STRICT_TASK_TYPES = {
...
def _load_criterion_templates() -> dict[str, dict[str, Any]]:
    ...
    path = ROOT / "01_semantic_knowledge/01_ontology/domains/semiconductor" / "business_instances.yaml"
        ...
        if criteria:
            ...
            for props in criteria:
                ...
                binding: dict[str, Any] = {
                    ...
                    "template_id": template_id,
                    "template_ref": (
                        "01_semantic_knowledge/01_ontology/domains/semiconductor/business_instances.yaml"
...
_EXTENSION_FILE = "domains/semiconductor/ontology_extension.yaml"
...
def _load_ontology_catalog(*, include_domain: bool = True) -> tuple[set[str], set[str], dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    paths = [ontology_root / "models" / filename for filename in _MODEL_FILES]
    if include_domain:
        paths.append(ontology_root / _EXTENSION_FILE)
    for path in paths:
        if not path.is_file():
            continue
        ...
        for section_name, bucket_objects, bucket_relations in (
            ...
            for item_id, definition in section.items():
                ...
                if isinstance(definition, dict):
                    ...
                    # operational.yaml uses simplified objects without metadata.status
                    if path.name == "operational.yaml" and not (isinstance(metadata, dict) and metadata.get("status")):
                        status = "active"
                if status != "active":
                    continue
                key = str(item_id)
