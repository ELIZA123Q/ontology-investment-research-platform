...
SAMPLE_ROOT = ROOT / "05_control_evaluation/04_verifiers/fixtures/reference_runs/research_runs"
...
_incremental_spec = importlib.util.spec_from_file_location(
    ...
    ROOT / "05_control_evaluation/04_verifiers/incremental_update.py",
)
...
def method_catalog_and_routes() -> tuple[dict[str, dict[str, str]], dict[str, Any]]:
    assets = load(ROOT / "03_agent_capability/02_skills/method_assets.yaml")
    structure_registry = load(ROOT / "03_agent_capability/02_skills/research_design/registry.yaml")
    evidence_registry = load(ROOT / "03_agent_capability/02_skills/evidence_research/registry.yaml")
    routes = load(ROOT / "05_control_evaluation/01_rules/policies/judgment_method_routes.yaml")
...
def ontology_catalog() -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]], set[str], set[str]]:
    ...
    core = ("semantic", "state_event", "evidence", "judgment", "operational")
    for name in core:
        document = load(ontology_root / "models" / f"{name}.yaml")
        objects.update(document.get("object_types") or {})
        relations.update(document.get("relation_types") or {})
    extension = load(ontology_root / "domains/semiconductor/ontology_extension.yaml")
    objects.update(extension.get("object_types") or {})
    relations.update(extension.get("relation_types") or {})
    domain_objects.update((extension.get("object_types") or {}).keys())
    event_extensions.update((extension.get("event_taxonomy") or {}).get("allowed_extensions") or [])
...
def validate_ontology_instances(structure: dict[str, Any], label: str) -> list[str]:
    ...
    expected_refs = {
        "01_semantic_knowledge/01_ontology/models/semantic.yaml",
        "01_semantic_knowledge/01_ontology/models/state_event.yaml",
        "01_semantic_knowledge/01_ontology/models/evidence.yaml",
        "01_semantic_knowledge/01_ontology/models/judgment.yaml",
        "01_semantic_knowledge/01_ontology/models/operational.yaml",
        "01_semantic_knowledge/01_ontology/domains/semiconductor/ontology_extension.yaml",
    }
    declared_refs = set(structure.get("ontology_refs") or [])
    # Accept legacy six-model refs during sample migration, but prefer the new bundle.
    legacy_refs = {
        "01_semantic_knowledge/01_ontology/models/semantic.yaml",
        "01_semantic_knowledge/01_ontology/models/state_event.yaml",
        "01_semantic_knowledge/01_ontology/models/evidence.yaml",
        "01_semantic_knowledge/01_ontology/models/judgment.yaml",
        "01_semantic_knowledge/01_ontology/models/scenario.yaml",
        "01_semantic_knowledge/01_ontology/models/semiconductor_extension.yaml",
    }
    if declared_refs not in (expected_refs, legacy_refs):
        errors.append(f"{label}: ontology_refs must declare the frozen model bundle (core + operational + semiconductor extension)")
    for ref in declared_refs:
        if ref in legacy_refs - expected_refs:
            continue  # legacy path retired; skip missing-file noise for historical samples
...
def known_rule_ids() -> set[str]:
    output: set[str] = set()
    for path in (ROOT / "01_semantic_knowledge/01_ontology/models").glob("*.yaml"):
        output.update((load(path).get("rules") or {}).keys())
    output.update((load(ROOT / "01_semantic_knowledge/01_ontology/domains/semiconductor/ontology_extension.yaml").get("evidence_constraints") or {}).keys())
    return output


def main(argv: list[str]) -> int:
    runs = tuple(Path(arg).resolve() for arg in argv) or discover_runs()
    contract = load(ROOT / "05_control_evaluation/01_rules/contracts/public_contract.yaml")
