...
GOVERNANCE_ONTOLOGY_ROOT = ROOT / "05_control_evaluation/06_change_governance/ontology"
...
FORMAL_PLATFORM_REGISTRY = ROOT / "01_semantic_knowledge/01_ontology/platform_registry.yaml"
...
def formal_model_files(platform_registry: dict[str, Any] | None = None) -> list[str]:
    registry = platform_registry or load(FORMAL_PLATFORM_REGISTRY)
    files = list(registry.get("semantic_models") or [])
    files.extend(registry.get("operational_models") or [])
...
def formal_element_ids(
    ...
    *,
    registry_path: Path = FORMAL_PLATFORM_REGISTRY,
) -> set[str]:
    del registry_path
    ids: set[str] = set()
    for raw_path in formal_model_files(model_registry):
        document = load(ONTOLOGY_DIR / str(raw_path))
...
def validate_governance_control_plane(
    ...
    *,
    ...
    formal_files = formal_model_files(formal_registry)
    ...
    if instance_graph.get("ontology_ref") != "05_control_evaluation/06_change_governance/ontology/model_registry.yaml":
...
def main() -> int:
    ...
    formal_registry = load(FORMAL_PLATFORM_REGISTRY)
