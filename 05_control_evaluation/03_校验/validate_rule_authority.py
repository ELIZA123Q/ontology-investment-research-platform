...
REGISTRY = ROOT / "05_control_evaluation/02_合同/rule_authority_registry.yaml"
OPERATIONS = ROOT / "01_semantic_knowledge/01_ontology/kinetics/action_types.yaml"
KINETIC_POLICIES = ROOT / "01_semantic_knowledge/01_ontology/kinetics/policies.yaml"
MODEL_FILES = tuple((ROOT / "01_semantic_knowledge/01_ontology/models").glob("*.yaml"))
SAMPLE_FILES = tuple((ROOT / "04_context_state/03_workspace/02_V3样例").glob("*/04_judgment.yaml"))
CURRENT_TEMPLATES = (
...
def validate_rule_authority(
    ...
    requirement_ids = {
        ...
        for rule in (load(ROOT / "05_control_evaluation/03_校验/requirements_coverage.yaml").get("rules") or [])
...
def main() -> int:
    ...
    print(
        ...
        f"samples={len(SAMPLE_FILES)}; 03_agent_capability/05_method_libraries/05_control_evaluation/runtime uniquely separated."
    )
    return 0
