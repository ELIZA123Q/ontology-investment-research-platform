...
VALIDATOR = ROOT / "05_control_evaluation/03_校验/validate_v3_samples.py"
...
class V3SampleTests(unittest.TestCase):
    def setUp(self) -> None:
        self.contract = validator.load(ROOT / "05_control_evaluation/02_合同/public_contract.yaml")
        self.rules = validator.known_rule_ids()
        self.run_dir = ROOT / "04_context_state/03_workspace/02_V3样例/01_memory-cycle-run-002"
    ...
    def test_examples_pass(self) -> None:
        ...
        other = validator.load_run(ROOT / "04_context_state/03_workspace/02_V3样例/02_us-controls-localization-run-002")
