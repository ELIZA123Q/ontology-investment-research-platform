...
SCENARIO_CATALOG = ROOT / "02_scenario_task" / "scenario_catalog.yaml"
PUBLIC_CONTRACT = ROOT / "05_control_evaluation/02_合同/public_contract.yaml"
MIGRATION_LEDGER = ROOT / "05_control_evaluation/05_元治理本体/迁移账本/2x_to_3_ledger.yaml"
RULE_REGISTRY = ROOT / "05_control_evaluation/02_合同/rule_authority_registry.yaml"
PSEUDO_TYPES = {
    "core_object",
    "stable_rule",
    "reasoning_object",
...
def validate_scenario_catalog(all_object_ids: set[str], scenario_contract: dict[str, Any]) -> list[str]:
    ...
    if not SCENARIO_CATALOG.exists():
        errors.append("02_scenario_task/scenario_catalog.yaml is missing")
        return errors
    catalog = load_yaml(SCENARIO_CATALOG)
    if catalog.get("enforcement") != "catalog_only":
        errors.append("scenario_catalog.enforcement must be catalog_only")
    ...
    for scenario_id, scenario in (catalog.get("scenario_types") or {}).items():
        missing_scenario = sorted(set(scenario_contract.get("required") or []) - set(scenario))
        if missing_scenario:
            errors.append(f"scenario_catalog:{scenario_id} missing {missing_scenario}")
        for key in set(scenario_contract.get("required") or []) - {"metadata"}:
            if not _list_of_nonempty_strings(scenario.get(key), allow_empty=False):
                errors.append(f"scenario_catalog:{scenario_id} {key} must be non-empty string list")
        if scenario.get("enforcement") != "catalog_only":
            errors.append(f"scenario_catalog:{scenario_id} enforcement must be catalog_only")
        for key in ("required_object_types", "required_state_types", "required_evidence_types", "required_judgment_types"):
            for ref in scenario.get(key) or []:
                if ref not in known:
                    errors.append(f"scenario_catalog:{scenario_id} unresolved type {ref}")
    return errors
...
def validate_migration_ledger(
    ...
    if (ROOT / "04_context_state/03_workspace/01_正式样例").exists():
