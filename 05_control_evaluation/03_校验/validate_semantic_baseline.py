...
PLATFORM_REGISTRY_PATH = ROOT / "01_semantic_knowledge/01_ontology/platform_registry.yaml"
...
def current_ontology_identity() -> tuple[str, list[str]]:
    ...
    registry = yaml.safe_load(PLATFORM_REGISTRY_PATH.read_text(encoding="utf-8"))
    if (
        not isinstance(registry, dict)
        or registry.get("schema_name") != "ontology_platform_registry"
        or registry.get("status") != "active"
    ):
        raise ValueError("正式本体 platform_registry 非法")
    model_files = [str(item) for item in (registry.get("semantic_models") or [])]
    model_files.extend(str(item) for item in (registry.get("operational_models") or []))
    for relative in model_files:
        path = ONTOLOGY_DIR / relative
        raw = path.read_bytes()
