...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "05_control_evaluation" / "03_校验" / "knowledge_graph"))
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "05_control_evaluation" / "03_校验" / "legacy_compat"))
...
TRACE_ID_RE = {
    "Q": re.compile(r"^Q-\d+$"),
    "JU": re.compile(r"^JU-\d+$"),
    "ER": re.compile(r"^ER-\d+$"),
    "CER": re.compile(r"^CER-\d+$"),
    "EV": re.compile(r"^EV-\d+$"),
    "C": re.compile(r"^C-(?:\d+|[A-Z0-9][A-Z0-9_-]*)$"),
    "RC": re.compile(r"^(?:RC|GATE|G)-\d+$"),
    "EX": re.compile(r"^EX-\d+$"),
}
CANONICAL_REQUIREMENT_RE = re.compile(r"^(?:ER|CER)-\d+$")
...
def require_trace_id(value: Any, kind: str, label: str) -> str:
    text = require_string(value, label)
    pattern = TRACE_ID_RE.get(kind)
    if pattern is None:
        fail(f"未知跨阶段 ID 类型: {kind}")
    if not pattern.match(text):
        fail(f"{label} 必须符合 {kind} 编号约定，当前为 {text}")
    return text


def is_canonical_requirement_id(value: str) -> bool:
    return bool(CANONICAL_REQUIREMENT_RE.match(str(value).strip()))
