...
SAMPLE_ROOT = ROOT / "04_context_state/03_workspace/02_V3样例"
...
_incremental_spec = importlib.util.spec_from_file_location(
    ...
    ROOT / "05_control_evaluation/03_校验/incremental_update.py",
)
...
def method_catalog_and_routes() -> tuple[dict[str, dict[str, str]], dict[str, Any]]:
    assets = load(ROOT / "03_agent_capability/05_method_libraries/00_登记/method_assets.yaml")
    structure_registry = load(ROOT / "03_agent_capability/05_method_libraries/02_研究框架/registry.yaml")
    evidence_registry = load(ROOT / "03_agent_capability/05_method_libraries/03_取证/03_registry.yaml")
    routes = load(ROOT / "05_control_evaluation/02_合同/judgment_method_routes.yaml")
...
def main(argv: list[str]) -> int:
    ...
    contract = load(ROOT / "05_control_evaluation/02_合同/public_contract.yaml")
