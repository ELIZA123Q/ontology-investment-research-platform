...
sys.path.insert(0, str(_ROOT / "05_control_evaluation" / "03_校验"))
...
METHOD_LIBRARY_ROOT = _ROOT / "03_agent_capability/05_method_libraries/04_裁决"
...
def _load_method_catalog() -> dict[str, Path]:
    ...
    if not METHOD_LIBRARY_ROOT.is_dir():
        fail(f"缺少03_agent_capability/05_method_libraries/04_裁决目录: {METHOD_LIBRARY_ROOT}")
    ...
    if not catalog:
        fail("03_agent_capability/05_method_libraries/04_裁决未发现任何 A 方法卡")
    return catalog
...
def _validate_method_library_usage(audit: dict[str, object]) -> None:
    ...
    try:
        ...
        fail("method_library_usage.library_ref 必须落在03_agent_capability/05_method_libraries/04_裁决目录内")
    ...
    if unknown:
        fail("method_library_usage.methods_used 未在03_agent_capability/05_method_libraries/04_裁决注册: " + ", ".join(unknown))
