...
TEMPLATE_DIR = WORKSPACE / "02_scenario_task/04_workflows/deep_research/templates/03_数据与证据快照模板"
STRATEGY_ROOT = WORKSPACE / "03_agent_capability/05_method_libraries/03_取证"
...
def _load_strategy_knowledge() -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    ...
    if not isinstance(registry, dict):
        fail("03_agent_capability/05_method_libraries/03_取证/03_registry.yaml 必须是 YAML 对象")
    ...
    for path in candidates:
        if path.is_file():
            try:
                ...
                fail(f"{label}.strategy_library_ref 必须落在03_agent_capability/05_method_libraries/03_取证目录内: {text}")
            return path
    fail(f"{label}.strategy_library_ref 无法解析到03_agent_capability/05_method_libraries/03_取证文件: {text}")
