...
FRAMEWORK_REGISTRY_PATH = WORKSPACE / "03_agent_capability/05_method_libraries/02_研究框架" / "registry.yaml"
SCENARIO_CARD_ROOT = WORKSPACE / "03_agent_capability/05_method_libraries/02_研究框架" / "行业"
...
def _assert_framework_asset_registered(framework_id: str, label: str, registered: set[str]) -> None:
    if framework_id not in registered:
        fail(f"{label}: {framework_id} 未在 03_agent_capability/05_method_libraries/02_研究框架 登记表或场景卡中定义")
...
def _validate_iteration_contract(view: dict[str, object]) -> None:
    ...
    if str(contract.get("convergence_contract_ref")) != "05_control_evaluation/02_合同/public_contract.yaml#iteration_semantics":
        fail("iteration_contract.convergence_contract_ref 必须引用公共合同 iteration_semantics")
    public = load_yaml_file(WORKSPACE / "05_control_evaluation/02_合同/public_contract.yaml")
