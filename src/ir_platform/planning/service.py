...
class ResearchPlanningService:
    ...
    def propose(self, request: dict[str, Any], state: dict[str, Any]) -> PlanProposal:
        ...
        context = {"request": request, "state": state, "methodology": methodology}
        matched = [
            result
        ...
        for item in selected_nodes:
            ...
            if item.capability_ref == "form_research_design":
                parameters.setdefault("research_design_contract", methodology["research_design_contract"])
                parameters.setdefault("causal_design_contract", methodology["causal_design_contract"])
            if item.capability_ref == "form_macro_context":
                parameters.setdefault(
                    "macro_context_contract",
                    {
                        **methodology["macro_context_contract"],
                        "expected_mode": methodology["macro_mode"],
                    },
                )
            if item.capability_ref == "package_evidence_handoff":
                parameters.setdefault(
                    "evidence_handoff_contract",
                    {
                        "information_cutoff": methodology["research_design_contract"]["information_cutoff"],
                        "required_for_hypotheses": True,
                    },
                )
            if item.capability_ref == "evaluate_causality":
                parameters.setdefault("causal_assessment_contract", methodology["causal_assessment_contract"])
            if item.capability_ref == "form_causal_judgment":
                parameters.setdefault("causal_contract_version", methodology["causal_contract_version"])
            if item.capability_ref in {"render_report", "publish_report"} and methodology["equity_report_contract"] is not None:
                parameters.setdefault("equity_report_contract", methodology["equity_report_contract"])
            if item.capability_ref == "validate_final_artifact":
                parameters.setdefault(
                    "final_artifact_validation_contract",
                    methodology["final_artifact_validation_contract"],
                )
            nodes.append(
                ProposedNode(
                    id=item.id,
