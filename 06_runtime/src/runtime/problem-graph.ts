...
export function selectTaskMotifs(goal: string): Motif[] {
...
export function buildResearchProblemGraph(input: {
  ...
  for (const motif of selectTaskMotifs(input.goal)) {
    const root = node(`${motif.ref}:question`, "root_question", `${motif.name}：${input.goal}`, true, motif.ref, { question: input.goal, scenarioRef: motif.scenario });
  ...
  return {
    ...
    taskMotifRefs: [...new Set(nodes.map((item) => item.motifRef).filter(Boolean) as string[])], nodes, edges,
  };
}
