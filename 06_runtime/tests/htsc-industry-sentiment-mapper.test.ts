...
const hash = `sha256:${createHash("sha256").update(rawResponseBody, "utf8").digest("hex")}`;
...
function receipt(overrides: Partial<HtscIndustrySentimentReceipt> = {}): HtscIndustrySentimentReceipt {
  return {
    ...
    providerResponseBody: rawResponseBody,
...
describe("HTSC industry sentiment mapper", () => {
  it("parses the real MCP markdown shape without model interpretation", () => {
    ...
    expect(parsed.responseFingerprint).toBe(hash);
  });
  ...
  it("preserves licensed provider identity, risk boundary, field lineage and business time", () => {
    ...
    expect(mapped.providerResponse).toMatchObject({ contentHash: hash, replayability: "time_sensitive" });
  ...
  it("rejects missing lineage, mismatched industries and unbound profiles", () => {
    ...
    expect(() => parseHtscIndustrySentimentMcpPayload({
      rawResponseBody: JSON.stringify({ status: "success", message: "查询成功", data: "no provider or risk" }),
      request: receipt().request, requestedAt: receipt().requestedAt, retrievedAt: receipt().retrievedAt,
