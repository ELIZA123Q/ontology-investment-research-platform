...
function fixture(): { root: string; store: RuntimeStore; semantic: LocalSemanticGateway; provenance: ResearchProvenanceStore; sources: LocalSourceGateway; capturePath: string } {
  ...
  mkdirSync(join(root, "04_context_state/03_workspace/03_回归/gold/02_assets/source_captures"), { recursive: true });
  ...
  const capturePath = join(root, "04_context_state/03_workspace/03_回归/gold/02_assets/source_captures/SR-01.json");
