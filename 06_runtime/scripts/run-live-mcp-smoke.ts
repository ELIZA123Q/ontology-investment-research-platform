...
import {
  ...
  type HtscIndustrySentimentMcpPayload,
...
const responseFingerprint = `sha256:${createHash("sha256").update(rawBytes).digest("hex")}`;
const payload = JSON.parse(rawBytes.toString("utf8")) as HtscIndustrySentimentMcpPayload;
