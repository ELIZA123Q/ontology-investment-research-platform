...
import type { Conversation, ReportAudience, ReportDepth, ReportKind, ReportSectionKey } from "@/src/contracts";
    ...
    <section className="home-grid">
      ...
      <aside className="home-side-column">
        <section className="section-block signal-block">
          ...
          {view && <div className={`connector-status ${view.connections.configured ? "configured" : "unconfigured"}`}><i/><div><strong>{view.connections.configured ? "外部连接器入口已启用" : "外部连接器入口未配置"}</strong><p>{view.connections.configured ? `${view.connections.allowedConnectorIds.length} 个允许的连接器 · 已接收 ${view.connections.sourceCaptureCount} 个来源批次、${view.connections.financialBatchCount} 个金融批次` : "配置服务端摄取令牌与连接器白名单后，实时材料才能进入证据链。"}</p>{view.connections.observedConnectorIds.length > 0 && <small>{view.connections.observedConnectorIds.join(" · ")}{view.connections.lastIngestedAt ? ` · 最近 ${formatRelativeTime(view.connections.lastIngestedAt)}` : ""}</small>}</div></div>}
          {view?.signals.available ? <div className="signal-candidate-list"><p>{view.signals.reason}</p>{view.signals.items.map((item) => <Link href={`/research/${item.conversationId}`} key={item.id}><div><strong>{item.title}</strong><small>{new Date(item.asOf).toLocaleDateString("zh-CN")} · 已校验候选观测</small></div><span>核验 →</span></Link>)}</div> : <div className="signal-unavailable"><i/><strong>{view?.connections.configured ? "等待连接器返回观测" : "实时来源尚未连接"}</strong><p>{view?.signals.reason || "正在确认数据连接状态…"}</p></div>}
        </section>
