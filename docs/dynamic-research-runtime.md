...
1. `ResearchMethodRegistry` 先解析任务显式声明的判断类型、取证方法、分析框架和领域目录；同时输出 `causal_identification_required` 与因果合同版本。未声明时保持为空，不根据标题关键词擅自套框架或因果方法。
...
机制验证、因果归因、传导路径和影响兑现会在证据评价后增加 `evaluate_causality`，并通过 `form_causal_judgment` 形成正式判断。设计阶段必须同时输出 `ResearchDesign` 与 `CausalDesign`；运行时校验 A→B、B→A、共同原因和可选反馈结构。非因果任务没有这两个节点。未启用因果流程的正式判断若使用“导致、造成、主要由”等强因果表达，会在写入前被拒绝。
