...
import { requireAssetPath, resolveAssetPath } from "./lib/asset_path";
import { registeredFiles } from "./skills/method_selection/knowledge_loader";
import type { StageKind } from "./schemas/types";

const COVERAGE_LOGICAL = "05_governance/03_校验/runtime_asset_coverage.yaml";
...
export function loadRuntimeAssetCoverage(): RuntimeAssetCoverage {
  const resolved = requireAssetPath(COVERAGE_LOGICAL);
  const raw = YAML.parse(readFileSync(resolved.absolutePath, "utf8")) as any;
  return {
    ...
    resolved_from: resolved.relativePath,
...
/** critical 资产 path 在仓库中存在（经 primary→compat 解析）。 */
export function missingCriticalPaths(
  ...
  return coverage.assets
    ...
    .map((item) => item.path)
    .filter((path) => !resolveAssetPath(path));
}
