...
const GLOBAL_AUTHORITY_REFS: AssetRef[] = [
  ["semantic", "ontology", "01_semantic/registry.yaml"],
  ["tasks", "workflow", "02_tasks/registry.yaml"],
  ["capabilities", "method", "03_capabilities/registry.yaml"],
  ["execution", "case", "04_execution/registry.yaml"],
  ["governance", "rule", "05_governance/registry.yaml"],
...
export class RuntimeStore {
  ...
  createTask(input: Omit<Task, "id" | "researchCaseId" | "createdAt" | "updatedAt"> & { researchCaseId?: string }): Task {
    const stamp = now();
    const id = randomUUID();
    const parentCaseId = input.parentTaskId ? this.getTask(input.parentTaskId)?.researchCaseId : undefined;
    const task: Task = { ...input, id, researchCaseId: input.researchCaseId || parentCaseId || id, createdAt: stamp, updatedAt: stamp };
    this.db.prepare(`INSERT INTO tasks
      (id,conversation_id,research_case_id,parent_task_id,goal,intent,status,budget_json,created_at,updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(task.id, task.conversationId, task.researchCaseId, task.parentTaskId ?? null, task.goal, task.intent, task.status, json(task.budget), stamp, stamp);
    return task;
  }
  ...
  reviseArtifact<T>(input: {
    ...
    this.db.exec("BEGIN IMMEDIATE");
    try {
      const current = this.getArtifact(input.id) as Artifact<T> | null;
      if (!current) throw new Error(`Artifact not found: ${input.id}`);
      if (current.version !== input.expectedVersion) {
        throw new ArtifactVersionConflictError(input.id, input.expectedVersion, current.version);
      }
