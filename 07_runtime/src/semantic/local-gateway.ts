...
const INDEX_ROOTS: ReadonlyArray<{ path: string; kind: SemanticAssetKind }> = [
  { path: "01_semantic/01_ontology", kind: "ontology" },
  { path: "01_semantic/02_dictionary", kind: "dictionary" },
  { path: "01_semantic/04_evidence", kind: "evidence_guide" },
  { path: "03_capabilities/05_method_libraries", kind: "method" },
  { path: "04_execution/03_workspace/03_回归", kind: "historical_artifact" },
];
...
export function resolveRepositoryRoot(start = process.cwd()): string {
  ...
  const found = candidates.find((candidate) => existsSync(resolve(candidate, "01_semantic/registry.yaml")));
