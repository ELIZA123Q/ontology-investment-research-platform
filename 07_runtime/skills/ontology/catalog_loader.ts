...
const modelRegistryResolved = requireAssetPath(
  "01_semantic/01_ontology/model_registry.yaml",
  ontologyModelRegistryCandidates(),
);
export const ONTOLOGY_MODEL_REGISTRY_FILE = modelRegistryResolved.relativePath;
export const ONTOLOGY_MODEL_REGISTRY_HIT = modelRegistryResolved.hit;

const modelRegistry = YAML.parse(readFileSync(modelRegistryResolved.absolutePath, "utf8")) as Record<string, unknown>;
if (
...
export function loadOntologyCatalog(): OntologyCatalog {
  ...
  for (const file of ONTOLOGY_MODEL_FILES) {
    const resolved = requireAssetPath(
      `01_semantic/01_ontology/models/${file}`,
      ontologyModelFileCandidates(file),
    );
    const sourceFile = resolved.relativePath;
    const rawText = readFileSync(resolved.absolutePath, "utf8");
...
export function loadSemiconductorBusinessObjectIds(): Set<string> {
  ...
  const resolved = requireAssetPath("01_semantic/01_ontology/domains/semiconductor/business_instances.yaml", {
    primary: ["01_semantic/01_ontology/domains/semiconductor/business_instances.yaml"],
  });
  const document = YAML.parse(readFileSync(resolved.absolutePath, "utf8")) as Record<string, any>;
