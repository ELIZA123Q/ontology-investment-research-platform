...
import { loadDataMappingRegistry } from "../financial_data/data_mapping";
import { loadMethodRegistry, inferJudgmentTypesFromTask, recallRegisteredMethodCandidates, type RegisteredMethod } from "./method_registry";
import { loadOntologyCatalog, ONTOLOGY_MODEL_FILES, ONTOLOGY_MODEL_REGISTRY_FILE, type OntologyElementDefinition } from "../ontology/catalog_loader";
import { parseJson, STAGES, type MethodApplication } from "../../schemas/types";
...
const METHOD_ASSET_ROOTS = ["90_compat/methods/02_研究框架", "90_compat/methods/03_取证", "90_compat/methods/04_裁决"];
const ALWAYS_INCLUDED_METHOD_FILES = [
  "90_compat/methods/README.md",
  "05_governance/05_元治理本体/method_assets.yaml",
  "90_compat/methods/02_研究框架/README.md",
  "90_compat/methods/02_研究框架/registry.yaml",
  "90_compat/methods/03_取证/README.md",
  "90_compat/methods/03_取证/03_registry.yaml",
  "90_compat/methods/03_取证/source_routes.yaml",
  "90_compat/methods/04_裁决/README.md",
  "05_governance/02_合同/judgment_method_routes.yaml",
];
...
function baselineFiles(): KnowledgePackageFile[] {
  ...
  const files: KnowledgePackageFile[] = [
    ...
    { file_name: "01_semantic/01_ontology/model_registry.yaml", content: readFileSync(repositoryPath(ONTOLOGY_MODEL_REGISTRY_FILE), "utf8") },
    ...ONTOLOGY_MODEL_FILES.map((file) => ({
      file_name: `01_semantic/01_ontology/models/${file}`,
      content: readFileSync(repositoryPath("01_semantic", "01_ontology", "models", file), "utf8"),
    })),
  ];
  const assets = new Set<string>([
    ...ALWAYS_INCLUDED_METHOD_FILES,
    ...METHOD_ASSET_ROOTS.flatMap(collectFiles),
    ...collectFiles("01_semantic/01_ontology/domains/semiconductor"),
...
function taskSliceFiles(runId: string): { files: KnowledgePackageFile[]; context: ReturnType<typeof selectedRunKnowledge> } {
  ...
  const mapping = loadDataMappingRegistry();
  const routeRegistry = YAML.parse(readFileSync(repositoryPath("05_governance/02_合同/judgment_method_routes.yaml"), "utf8")) as Record<string, any>;
