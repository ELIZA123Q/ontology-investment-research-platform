...
import { repositoryPath, repositoryRoot } from "../../storage/repo_paths";
...
import type { MethodCapabilityType } from "../../schemas/types";
...
export const SEMICONDUCTOR_SCENARIO_CARDS: ScenarioCardSpec[] = [
  {
    ...
    file: "90_compat/methods/02_研究框架/行业/半导体/场景卡/SCN-MEM-CYCLE_存储周期见顶或反转场景卡.md",
    keywords: ["存储周期", "DRAM", "NAND", "见顶", "周期反转", "内存周期", "存储芯片周期", "稀缺定价"],
  },
  {
    scenario_id: "SCN-MEM-HBM",
    file: "90_compat/methods/02_研究框架/行业/半导体/场景卡/SCN-MEM-HBM_HBM供需与资源挤占场景卡.md",
...
/** Map BF-/IF- ids to framework markdown paths under 90_compat/methods/02_研究框架. */
export function frameworkMarkdownIndex(): Map<string, string> {
  if (frameworkMarkdownCache) return frameworkMarkdownCache;
  const roots = [
    "90_compat/methods/02_研究框架/基础",
    "90_compat/methods/02_研究框架/行业",
  ];
...
export function enrichPromptMethodCards(
  ...
  const frameworkRegistry = parseYaml("90_compat/methods/02_研究框架/registry.yaml");
  const evidenceRegistry = parseYaml("90_compat/methods/03_取证/03_registry.yaml");
  const frameworks = {
...
export function judgmentThresholdCapsForPrompt(): ThresholdCapsProjection {
  const policy = parseYaml("05_governance/02_合同/judgment_threshold_policy.yaml");
  return {
...
export function evidenceJudgmentTypeCardsForPrompt(judgmentTypes: string[]) {
  const registry = parseYaml("90_compat/methods/03_取证/03_registry.yaml");
