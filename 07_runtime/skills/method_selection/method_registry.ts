...
import type { MethodApplication, MethodCapabilityType } from "../../schemas/types";
...
export function loadMethodRegistry(): Map<string, RegisteredMethod> {
  const root = parseYaml("05_governance/knowledge_changes/method_assets.yaml") as MethodAssetRegistry;
  ...
  if (evidence?.registry_ref) {
    ...
    for (const [localId, method] of Object.entries<any>(registry.methods || {})) {
      ...
      result.set(methodId, {
        ...
        file: method.file ? `90_compat/methods/03_取证/${method.file}` : evidence.registry_ref,
  ...
  for (const method of adjudication?.methods || []) {
    result.set(method.id, {
      ...
      source_ref: `05_governance/knowledge_changes/method_assets.yaml#groups.adjudication.methods.${method.id}`,
...
export function methodRoutesForPrompt() {
  const routeRegistry = parseYaml("05_governance/02_合同/judgment_method_routes.yaml");
