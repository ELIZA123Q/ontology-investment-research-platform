...
import "server-only";
import { latestArtifact, listSources, quarantineUnboundWebCitations } from "../storage/db";
import { assembleStageContext, buildSemanticRoute, clipUpstreamJsonSoft, CONTEXT_SLOT_BUDGETS } from "./context_assembler";
import { evidenceBoundSourceIds, evidenceBoundSources } from "../skills/evidence_evaluation/sources";
import { buildGovernanceFingerprint } from "../governance_fingerprint";
import { formalOntologyRuleIds } from "../agents/04_judgment/draft_normalize";
import {
...
import { methodRoutesForPrompt } from "../skills/method_selection/method_registry";
import { loadOntologyCatalog } from "../skills/ontology/catalog_loader";
import { ontologyContextForPrompt, ontologyPromptSourceFiles } from "../skills/ontology/tools";
import { promptFor } from "../agents/shared/prompts_source";
import { summarizeInjectedAssets } from "../runtime_asset_coverage";
import { pendingClarificationQuestion } from "../agents/01_intake/input_contract";
import { parseJson, type ArtifactKind, type StageKind } from "../schemas/types";
import {
...
import { compactStage03ForUpstream, compactStage04ForStage05, compactStructuredArtifact } from "./support";
  ...
  const inputContext = JSON.stringify(
    {
      ...
      delivery_archetype: kind === "stage_05" ? {
        ...
        template_hint: deliveryArchetype === "industry_cycle_report"
          ? "90_compat/knowledge/expression/templates/05C_行业周期判断模板.md"
          : deliveryArchetype === "event_commentary"
            ? "90_compat/knowledge/expression/templates/05A_事件点评模板.md"
            : deliveryArchetype === "industry_dynamic_commentary"
              ? "90_compat/knowledge/expression/templates/05B_行业动态点评模板.md"
              : deliveryArchetype === "company_earnings_commentary"
                ? "90_compat/knowledge/expression/templates/05D_公司业绩点评模板.md"
                : deliveryArchetype === "theme_deep_dive"
                  ? "90_compat/knowledge/expression/templates/05E_主题深度研究模板.md"
                  : "90_compat/knowledge/expression/templates/05C_行业周期判断模板.md",
        expression_standard: "90_compat/knowledge/expression/standards/05_投研表达标准.md",
