...
import type { AssetRef } from "@/src/contracts";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await context.params;
    const body = await request.json() as { content?: string; pinnedAssetRefs?: AssetRef[] };
    ...
    const result = kernel.submitGoal(id, body.content.trim(), planning.proposal, { pinnedAssetRefs: Array.isArray(body.pinnedAssetRefs) ? body.pinnedAssetRefs : [] });
