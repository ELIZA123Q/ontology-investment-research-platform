...
export default function KnowledgeLibraryQualityPage() {
  ...
  const issues = [
    { id: "endpoints", severity: data.danglingRelations.length || data.duplicateOntologyIds ? "high" : "pass", title: "目录、重复 ID 与关系端点", result: data.danglingRelations.length || data.duplicateOntologyIds ? `${data.duplicateOntologyIds} 个重复 ID · ${data.danglingRelations.length} 条悬空关系` : "检查通过", action: "定位知识地图", href: "/90_compat/knowledge/library/map" },
    { id: "legacy", severity: data.affectedLegacyRuns.length ? "medium" : "pass", title: "研究基线一致性", result: data.affectedLegacyRuns.length ? `${data.affectedLegacyRuns.length} 个研究仍绑定旧指纹` : "全部采用当前指纹", action: "查看受影响研究", href: "/90_compat/knowledge/library/usage" },
    { id: "mapping", severity: data.activeMappings >= data.mappings.required_connectors.length ? "pass" : "high", title: "数据映射完整性", result: `${data.activeMappings}/${data.mappings.required_connectors.length} 个必需通道活动`, action: "检查映射定义", href: "/90_compat/knowledge/library/usage" },
    { id: "candidate", severity: data.repeatedCandidates.length ? "medium" : "pass", title: "重复非正式概念", result: data.repeatedCandidates.length ? `${data.repeatedCandidates.length} 项跨研究重复` : "没有跨研究重复候选", action: "进入缺口治理", href: "/90_compat/knowledge/library/gaps" },
    { id: "unused", severity: data.unusedFormalCount ? "low" : "pass", title: "未使用正式知识", result: `${data.unusedFormalCount} 项未观察到实际触及`, action: "在应用表筛选", href: "/90_compat/knowledge/library/usage" },
  ];
