  ...
  return <main className={`workspace-page ${dockOpen ? "dock-open" : ""}`} id="main-content">
    ...
    <section className={`conversation-column ${mobileView === "conversation" ? "mobile-active" : ""}`}>
      <div className="conversation-scroll">
        ...
        {snapshot.approvals.map((approval) => <article className="approval-card" key={approval.id}>
          <div className="approval-kicker">需要你的判断</div><h3>{approval.prompt}</h3><p>{approval.kind === "plan_confirmation" ? "计划只包含受约束节点；确认后交给独立 Worker 执行。" : "请先在右侧核对或修改对应制品。确认后才会继续生成下游结果。"}</p>
          <div><button disabled={busy} onClick={() => void decide(approval, "rejected")}>{approval.kind === "plan_confirmation" ? "调整计划" : "退回修改"}</button><button className="primary-button" disabled={busy} onClick={() => void decide(approval, "approved")}>{approval.kind === "plan_confirmation" ? "确认开始" : "确认并继续"}<ArrowIcon /></button></div>
        </article>)}
      </div>
