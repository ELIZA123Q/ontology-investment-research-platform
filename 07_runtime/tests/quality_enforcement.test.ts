...
describe("quality enforcement acceptance", () => {
  ...
  it("01–04 knowledge injects 00A card and key appendices", () => {
    expect(registeredFiles("stage_01")).toContain("05_governance/03_校验/00A_runtime_quality_card.md");
    expect(registeredFiles("stage_02")).toContain(
      "02_tasks/04_workflows/deep_research/supporting/02_structure_ontology_gap_matrix.md",
    );
    expect(registeredFiles("stage_03")).toContain(
      "02_tasks/04_workflows/deep_research/supporting/03_evidence_provenance_manual.md",
    );
    expect(registeredFiles("stage_04")).toContain("05_governance/03_校验/00A_runtime_quality_card.md");
  });

  it("validate_run.py remains the formal publish authority", () => {
    const root = join(__dirname, "../..");
    const validateRun = join(root, "05_governance/03_校验/validate_run.py");
