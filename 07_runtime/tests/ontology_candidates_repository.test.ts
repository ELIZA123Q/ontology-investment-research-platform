...
let repository: typeof import("@/05_governance/ontology_changes/candidates_adapter");

beforeAll(async () => {
  db = await import("@/storage/db");
  repository = await import("@/05_governance/ontology_changes/candidates_adapter");
});
...
describe("ontology candidate governance repository", () => {
  ...
  it("does not confuse an accepted candidate with a formally released ontology element", async () => {
    ...
    repository.applyOntologyGovernanceAction({
      ...
      implementationRef: "01_semantic/01_ontology/domains/semiconductor/business_instances.yaml#inventory_cycle",
    });
    repository.applyOntologyGovernanceAction({
      requestId: proposed.id,
