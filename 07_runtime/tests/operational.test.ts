...
let actionExecutor: typeof import("@/05_governance/ontology_changes/action_executor");
...
beforeAll(async () => {
  ...
  actionExecutor = await import("@/05_governance/ontology_changes/action_executor");
...
describe("v1.3 operational spine", () => {
  ...
  it("separates formal ontology nodes from runtime operations", () => {
    ...
    expect(actions.every((action) => action.source_file === "90_compat/governance_mirrors/contracts/runtime_operations.yaml")).toBe(true);
  ...
  it("queries package-bound object set", () => {
    const run = db.createRun("对象集测试", "semiconductor", "90_compat/instances/02_V3样例/01_memory-cycle-run-002");
