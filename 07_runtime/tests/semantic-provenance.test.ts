...
function fixture(): { root: string; store: RuntimeStore; semantic: LocalSemanticGateway; provenance: ResearchProvenanceStore; sources: LocalSourceGateway; capturePath: string } {
  ...
  mkdirSync(join(root, "01_semantic/01_ontology"), { recursive: true });
  mkdirSync(join(root, "01_semantic/02_dictionary"), { recursive: true });
  mkdirSync(join(root, "04_execution/03_workspace/03_回归/gold/02_assets/source_captures"), { recursive: true });
  writeFileSync(join(root, "01_semantic/registry.yaml"), "schema_name: semantic\n");
  writeFileSync(join(root, "01_semantic/01_ontology/hbm.yaml"), "id: HBM\nlabel: 高带宽存储\nrelation: AI算力需求\n");
  const capturePath = join(root, "04_execution/03_workspace/03_回归/gold/02_assets/source_captures/SR-01.json");
