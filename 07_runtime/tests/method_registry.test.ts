...
describe("runtime knowledge contexts", () => {
  it("loads stage_01 template and stage_02 appendix", () => {
    expect(registeredFiles("stage_01")).toEqual(expect.arrayContaining([
      "02_tasks/04_workflows/deep_research/templates/01_intake_brief_template.md",
    ]));
    expect(registeredFiles("stage_02")).toEqual(expect.arrayContaining([
      "02_tasks/04_workflows/deep_research/supporting/02_structure_framework_selection.md",
    ]));
  });
  ...
  it("loads B00/B01/B03/OPS for stage_03 and omits routes yaml from stage_04 knowledge", () => {
    expect(registeredFiles("stage_03")).toEqual(expect.arrayContaining([
      "01_semantic/04_evidence/B00_来源选择与使用边界.md",
      "01_semantic/04_evidence/B01_通用来源速查.md",
      "03_capabilities/04_protocols/mcp/B03_MCP通道注册.md",
      "03_capabilities/04_protocols/mcp/OPS_MCP查询快速参考.md",
    ]));
    expect(registeredFiles("stage_04")).not.toContain("05_governance/02_合同/judgment_method_routes.yaml");
    expect(registeredFiles("stage_04")).not.toContain("90_compat/governance_mirrors/contracts/judgment_method_routes.yaml");
  });
  ...
  it("injects the blocking protocol for all Stage04 judgment types", () => {
    ...
    expect(without).toContain("90_compat/methods/04_裁决/A00-附录1_路径与阻断协议.md");
    expect(withPath).toContain("90_compat/methods/04_裁决/A00-附录1_路径与阻断协议.md");
  });
  ...
  it("loads only the matching stage_05 archetype template", () => {
    ...
    expect(cycle.filter((file) => /methods\/05_表达\/templates\/05[A-E]_/.test(file))).toEqual([
      "90_compat/methods/05_表达/templates/05C_行业周期判断模板.md",
    ]);
    expect(event.filter((file) => /methods\/05_表达\/templates\/05[A-E]_/.test(file))).toEqual([
      "90_compat/methods/05_表达/templates/05A_事件点评模板.md",
    ]);
  });
});
