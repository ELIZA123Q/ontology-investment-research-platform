...
describe("approved semantic snapshot authority", () => {
  ...
  it("verifies an approved snapshot against the authority graph and rejects later drift", () => {
    ...
    const snap = reads.loadApprovedSemanticSnapshot(run.id, "stage_02");
    expect(snap.data._projected_from_graph).toBe(true);
    expect(snap.data.judgment_units?.[0]?.id).toBe("JU-1");
