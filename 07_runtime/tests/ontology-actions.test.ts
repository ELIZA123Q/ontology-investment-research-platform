...
describe("Ontology 4.0 action platform", () => {
  ...
  it("enforces capture-to-fact provenance and researcher approval before supported judgment", () => {
    ...
    const preview = actions.preview("ApproveJudgment", {
      targetRefs: [{ id: researchCase.id, type: researchCase.type }], parameters: { statement: "需求有条件增长", epistemicStatus: "supported", confidence: "medium", scopeRef: researchCase.id, cutoffAt: "2026-08-02T00:00:00Z", evidenceRefs: [fact.id], hypothesisRefs: [], conditions: ["一手来源持续"], invalidationConditions: ["新来源反转"] },
      expectedVersions: { [researchCase.id]: researchCase.version }, idempotencyKey: "judgment:preview",
    }, context);
    ...
    expect(() => actions.apply("ApproveJudgment", {
      targetRefs: [{ id: researchCase.id, type: researchCase.type }], parameters: { statement: "需求有条件增长", epistemicStatus: "supported", confidence: "medium", scopeRef: researchCase.id, cutoffAt: "2026-08-02T00:00:00Z", evidenceRefs: [fact.id], hypothesisRefs: [], conditions: ["一手来源持续"], invalidationConditions: ["新来源反转"] },
      expectedVersions: { [researchCase.id]: researchCase.version }, idempotencyKey: "judgment:no-approval",
    }, context)).toThrow(ActionRejectedError);
    store.decideApproval(approval.id, "approved");
    const result = actions.apply("ApproveJudgment", {
      targetRefs: [{ id: researchCase.id, type: researchCase.type }], parameters: { statement: "需求有条件增长", epistemicStatus: "supported", confidence: "medium", scopeRef: researchCase.id, cutoffAt: "2026-08-02T00:00:00Z", evidenceRefs: [fact.id], hypothesisRefs: [], conditions: ["一手来源持续"], invalidationConditions: ["新来源反转"] },
      expectedVersions: { [researchCase.id]: researchCase.version }, idempotencyKey: "judgment:approved", approvalToken: approval.id,
  ...
  it("invalidates downstream formal facts and judgments when a source gets a new snapshot", () => {
    ...
    const judgment = actions.apply("ApproveJudgment", { targetRefs: [{ id: researchCase.id, type: researchCase.type }], parameters: { statement: "增长", epistemicStatus: "supported", confidence: "medium", scopeRef: researchCase.id, cutoffAt: "2026-08-01T00:00:00Z", evidenceRefs: [fact.id], hypothesisRefs: [], conditions: [], invalidationConditions: ["新版本"] }, expectedVersions: { [researchCase.id]: 1 }, idempotencyKey: "judgment:v1", approvalToken: approval.id }, { ...context, actorType: "researcher" }).objects.find((object) => object.type === "Judgment")!;
