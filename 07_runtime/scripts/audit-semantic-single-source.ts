...
const registry = YAML.parse(read("01_semantic/01_ontology/model_registry.yaml")) as Record<string, unknown>;
...
for (const file of modelFiles) {
  try {
    read(`01_semantic/01_ontology/${file}`);
  } catch {
...
for (const absolute of runtimeProductionFiles) {
  ...
  if (
    source.includes("01_semantic/01_ontology/models")
...
const mapping = YAML.parse(read("90_compat/governance_mirrors/contracts/ontology_data_mapping_profiles.yaml")) as Record<string, any>;
...
const mcpSource = read("07_runtime/skills/financial_data/mcp_registry.ts");
...
const requiredMarkers: Array<[string, string, string]> = [
  ["07_runtime/workflow/orchestrator.ts", "buildStageSemanticContext", "生成阶段语义封套"],
  ["07_runtime/workflow/approval.ts", "buildStageSemanticContext", "批准阶段语义封套"],
  ["07_runtime/export/formal_pack_export.ts", "buildProductionSemanticBaseline", "生产正式发布语义基线"],
  ["05_governance/03_校验/validate_run.py", "validate_semantic_baseline", "正式发布 Python 语义门"],
  ["07_runtime/05_governance/ontology_changes/action_executor.ts", "assertFormalRelationChoice", "关系写入正式端点门"],
  ["07_runtime/skills/ontology/semantic_reads.ts", "formal_graph_verified", "已批准阶段语义读取门"],
];
...
checks.approved_stage_read_audit = {
  ...
  guarded_consumers: [
    ...
    "06_app/api/runs/[id]/evaluation/route.ts",
    "06_app/api/runs/[id]/evaluation/recompute-metrics/route.ts",
