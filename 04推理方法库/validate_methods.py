...
def check_path_status_contract(errors: list[str]) -> None:
    b01_path = ROOT / "B_推理路径验证/B01_路径成立与逐段验证.md"
    b01_text = b01_path.read_text(encoding="utf-8")
    b01_codes = table_status_codes(b01_text)
    if b01_codes != PATH_STATUS_CODES:
        missing = sorted(PATH_STATUS_CODES - b01_codes)
        extra = sorted(b01_codes - PATH_STATUS_CODES)
        errors.append(f"B01 路径状态值域不一致：缺少 {missing or '无'}；多出 {extra or '无'}")
    if re.search(r"(?m)^\|\s*条件成立\s*\|", b01_text):
        errors.append("B01 不得把“条件成立”定义为正式路径状态")

    output_spec = (PROJECT_ROOT / "04_推理输出规范.md").read_text(encoding="utf-8")
    output_codes = table_status_codes(output_spec)
    missing_in_spec = PATH_STATUS_CODES - output_codes
    if missing_in_spec:
        errors.append(f"04_推理输出规范.md 缺少 B01 正式状态码：{sorted(missing_in_spec)}")

    status_definition_files = []
    for path in ROOT.glob("[ABCD]_*/*.md"):
        if re.search(r"(?m)^##\s+路径状态\s*$", path.read_text(encoding="utf-8")):
...
def main() -> int:
    ...
    check_path_status_contract(errors)
