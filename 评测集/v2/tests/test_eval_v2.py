#!/usr/bin/env python3
"""Contract, failure-mode, metric and deterministic end-to-end tests for eval v2."""
...
def run_cli(*args: str, expect: int = 0) -> subprocess.CompletedProcess[str]:
    completed = subprocess.run(
        [sys.executable, str(V2_ROOT / "eval_cli.py"), *args],
        cwd=REPO_ROOT,
        text=True,
...
class ContractTests(unittest.TestCase):
    def test_suite_and_defects_validate(self) -> None:
        ...
        defects = load_yaml(V2_ROOT / "calibration" / "defects.yaml")["items"]
    ...
    def test_official_profile_separation_fails(self) -> None:
        source = load_yaml(V2_ROOT / "mock_profiles.yaml")
        with tempfile.TemporaryDirectory() as raw:
...
class EndToEndTests(unittest.TestCase):
    ...
    def test_mock_pipeline_resume_and_unordered_aggregation(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertNotIn("综合分", (run_dir / "report.md").read_text(encoding="utf-8"))
