...
def command_validate(args: argparse.Namespace) -> int:
    ...
    print(
        f"V2_EVAL_PASS: {len(cases)}个试点案例，{evidence_count}条冻结证据，"
...
def command_prepare(args: argparse.Namespace) -> int:
    ...
    for info in cases:
        ...
        (case_dir / "C.pending").write_text("direct report is generated during run\n", encoding="utf-8")
...
def command_run(args: argparse.Namespace) -> int:
    ...
    if orchestrator.calibration_blocked:
        print(
            f"V2_CALIBRATION_BLOCKED: requests={len(orchestrator.existing)} errors={orchestrator.errors}",
            file=sys.stderr,
        )
        return 3
    print(
        f"V2_RUN_COMPLETE: stage={args.stage} requests={len(orchestrator.existing)} errors={orchestrator.errors}"
    )
...
def compute_case_summary(case_id: str, info: dict[str, Any], rows: list[dict[str, Any]], calibration: dict[str, Any]) -> dict[str, Any]:
    ...
    proxy_interval = wilson_interval(successes, len(case_levels))
    proxy = None
    if C_ORDER[calibration["grade"]] >= C_ORDER["C2"] and calibration["all_judges_eligible"]:
        proxy = {
            "estimate": round(successes / len(case_levels), 4),
            "wilson_90": [round(proxy_interval[0], 4), round(proxy_interval[1], 4)],
            "label": "代理可靠概率，不是客观正确率",
        }
    ...
    task_delta_pp = {
        baseline: round((context_means.get("D", 0.0) - context_means.get(baseline, 0.0)) * 100, 2)
    ...
    for baseline in ["B", "C"]:
        base = error_means.get(baseline, 0.0)
        current = error_means.get("D", 0.0)
        error_reduction[baseline] = {
            "total": round((base - current) / base, 4) if base > 0 else 0.0
        }
        for category in ["fact", "reasoning", "extrapolation"]:
            base_category = safe_mean(error_categories[baseline][category])
            current_category = safe_mean(error_categories["D"][category])
            error_reduction[baseline][category] = (
    ...
    for baseline in ["direct_report", "evidence_summary"]:
        selected_rows = [
            row
    ...
    delta_gates = {
        "direct_report_win_rate_at_least_60pct": pairwise["direct_report"]["D_win_rate"] >= 0.60,
        "evidence_summary_win_rate_at_least_55pct": pairwise["evidence_summary"]["D_win_rate"] >= 0.55,
        "task_gain_vs_direct_positive": task_delta_pp["C"] > 0,
        "errors_not_increased": all(
            error_reduction[baseline]["total"] >= 0 for baseline in ["B", "C"]
        ),
    }
    ...
    return {
        ...
        "R": f"R{r_value}",
        "proxy_reliability_probability": proxy,
        "U": u_grade,
        "delta": {
        ...
        "diagnostics": {
            ...
            "applicability": "冻结证据边界内的四题校准试点，不代表未知任务泛化能力",
        },
    }
...
def command_aggregate(args: argparse.Namespace) -> int:
    ...
    for stratum in ["report_value", "restraint"]:
        ...
        strata[stratum] = {
            ...
            "direct_report_win_rate_mean": round(
                safe_mean([item["delta"]["pairwise"]["direct_report"]["D_win_rate"] for item in selected]), 4
            ),
            "evidence_summary_win_rate_mean": round(
                safe_mean([item["delta"]["pairwise"]["evidence_summary"]["D_win_rate"] for item in selected]), 4
            ),
        }
    result = {
        ...
        "official_probability_release_allowed": (
            C_ORDER[calibration["grade"]] >= C_ORDER["C2"] and calibration["all_judges_eligible"]
        ),
        ...
        "strata": strata,
        "limitations": [
            "代理可靠概率只反映六次AI裁决在冻结证据边界内的频率。",
            "四题为校准试点，report_value与restraint不得混报为泛化通过率。",
    ...
    print(f"V2_AGGREGATED: C={calibration['grade']} cases={len(summaries)}")
    return 0
...
def command_report(args: argparse.Namespace) -> int:
    ...
    lines = [
        ...
        f"- 评测可信度：**{summary['calibration']['grade']}**",
        f"- 专家盲审：`{summary['expert_review']['status']}`（首版非前置条件）",
        f"- 是否允许展示代理可靠概率：{'是' if summary['official_probability_release_allowed'] else '否'}",
        "- 本报告不提供综合总分。",
        "",
    ]
    for case_id, item in summary["cases"].items():
        lines.extend([f"## {case_id} · {item['stratum']}", ""])
        lines.append(f"- 研究判断可靠性：**{item['R']}**")
        proxy = item.get("proxy_reliability_probability")
        if proxy:
            low, high = proxy["wilson_90"]
            lines.append(
                f"- 代理可靠概率：{percent(proxy['estimate'])}，Wilson 90%区间 {percent(low)}—{percent(high)}"
            )
        else:
            lines.append("- 代理可靠概率：因C低于C2而抑制")
        direct_pair = item["delta"]["pairwise"]["direct_report"]
        ...
        direct_errors = item["delta"]["error_reduction_rate"]["C"]["total"]
        summary_errors = item["delta"]["error_reduction_rate"]["B"]["total"]
        lines.extend(
            [
                f"- 任务有用性：**{item['U']}**",
                (
                    f"- 相对直接生成稿：任务增益 {item['delta']['task_score_pp']['C']:+.1f}pp；"
                    f"错误降低 {percent(direct_errors)}；盲评胜率 {percent(direct_pair['D_win_rate'])} "
                    f"（Wilson 90%区间 {percent(direct_low)}—{percent(direct_high)}）"
                ),
                (
                    f"- 相对证据摘要稿：任务增益 {item['delta']['task_score_pp']['B']:+.1f}pp；"
                    f"错误降低 {percent(summary_errors)}；盲评胜率 {percent(summary_pair['D_win_rate'])} "
                    f"（Wilson 90%区间 {percent(summary_low)}—{percent(summary_high)}）"
                ),
                f"- 相对仅问题组任务增益：{item['delta']['task_score_pp']['A']:+.1f}pp",
                f"- 稳定性：**{item['S']}**",
                f"- 硬错误：{'；'.join(item['diagnostics']['hard_failures']) if item['diagnostics']['hard_failures'] else '无'}",
                f"- 最弱核心判断：{item['diagnostics']['weakest_core_judgment']}",
                f"- 最强反证：{item['diagnostics']['strongest_counterevidence']}",
                f"- 主要分歧：{item['diagnostics']['major_disagreement']}",
                f"- 适用边界：{item['diagnostics']['applicability']}",
                "",
            ]
        )
    lines.extend(["## 评测器校准", ""])
    ...
    print(f"V2_REPORT_WRITTEN: {report_path}")
    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="研究价值代理评测体系v2")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"V2_EVAL_FAIL: {exc}", file=sys.stderr)
        return 1
