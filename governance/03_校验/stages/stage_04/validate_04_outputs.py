...
sys.path.insert(0, str(_ROOT / "governance" / "03_校验"))
sys.path.insert(0, str(_ROOT / "runtime" / "adapters"))
...
METHOD_LIBRARY_ROOT = _ROOT / "methods/04_裁决"
...
def _load_method_catalog() -> dict[str, Path]:
    ...
    if not METHOD_LIBRARY_ROOT.is_dir():
        fail(f"缺少methods/04_裁决目录: {METHOD_LIBRARY_ROOT}")
...
def _validate_method_library_usage(audit: dict[str, object]) -> None:
    ...
    try:
        ...
        fail("method_library_usage.library_ref 必须落在methods/04_裁决目录内")
    ...
    if unknown:
        fail("method_library_usage.methods_used 未在methods/04_裁决注册: " + ", ".join(unknown))
