...
sys.path.insert(0, str(Path(__file__).resolve().parents[4] / "governance" / "03_校验"))
...
STRATEGY_ROOT = WORKSPACE / "methods/03_取证"
...
def _load_strategy_knowledge() -> tuple[dict[str, object], dict[str, object], dict[str, object]]:
    ...
    if not isinstance(registry, dict):
        fail("methods/03_取证/03_registry.yaml 必须是 YAML 对象")
    ...
    for path in candidates:
        if path.is_file():
            try:
                ...
                fail(f"{label}.strategy_library_ref 必须落在methods/03_取证目录内: {text}")
            return path
    fail(f"{label}.strategy_library_ref 无法解析到methods/03_取证文件: {text}")
