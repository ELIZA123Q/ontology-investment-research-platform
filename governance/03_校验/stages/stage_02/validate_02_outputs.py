...
FRAMEWORK_REGISTRY_PATH = WORKSPACE / "methods/02_判断结构" / "00_framework_dependency_registry.yaml"
SCENARIO_CARD_ROOT = WORKSPACE / "methods/02_判断结构" / "行业框架库"
...
def _assert_framework_asset_registered(framework_id: str, label: str, registered: set[str]) -> None:
    if framework_id not in registered:
        fail(f"{label}: {framework_id} 未在methods/02_判断结构登记表或场景卡中定义")
