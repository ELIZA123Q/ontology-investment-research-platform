...
def validate_repository() -> list[str]:
    ...
    required_spec_markers = {
        ...
        "05": ("EX → C", "递归拒绝", "只读子集", "delivery.template:05F", "CEO战略报告"),
    }
    ...
    ceo_template = ROOT / "delivery/02_模板/05F_CEO战略报告模板.md"
    if not ceo_template.is_file():
        errors.append("delivery template 05F CEO strategy report is missing")
    else:
        template_text = ceo_template.read_text(encoding="utf-8")
        for marker in ("ceo_strategy_report: true", "## 摘要", "## 附录"):
            if marker not in template_text:
                errors.append(f"05F CEO strategy report template lacks {marker}")
    return errors
