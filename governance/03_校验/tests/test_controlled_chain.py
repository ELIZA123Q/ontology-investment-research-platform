...
class ControlledChainTests(unittest.TestCase):
    def test_normal_support_chain_is_publishable(self) -> None:
        completed = subprocess.run(
            [sys.executable, "governance/03_校验/validate_run.py", "instances/01_正式样例/02_管制与国产替代", "--no-write"],
            cwd=ROOT,
            check=False,
            capture_output=True,
            text=True,
    ...
    def test_allowed_alternative_evidence_method_requires_reason(self) -> None:
        ...
        routes = validate_evidence_routes(_view("instances/01_正式样例/01_存储周期"), rows)
    ...
    def test_risk_reassessment_must_return_to_02_for_decomposition(self) -> None:
        view = copy.deepcopy(_view("instances/01_正式样例/01_存储周期"))
    ...
    def test_strict_j4_passes_and_missing_upgrade_review_fails(self) -> None:
        view = _view("instances/01_正式样例/02_管制与国产替代")
        audit = _audit("instances/01_正式样例/02_管制与国产替代")
    ...
    def test_05_overreach_is_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            source_dir = ROOT / "instances/01_正式样例/02_管制与国产替代"
            ...
            completed = subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "governance/03_校验/stages/stage_05" / "validate_05_outputs.py"),
                    str(delivery),
                    "主题深度研究",
                    str(expression_copy),
                    str(audit),
    ...
    def test_upstream_change_marks_downstream_stale(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            shutil.copytree(ROOT / "instances/01_正式样例/01_存储周期", target)
            view_path = next(target.glob("02-*本体视图-*.yaml"))
            view_path.write_text(view_path.read_text(encoding="utf-8") + "\n", encoding="utf-8")
            completed = subprocess.run(
                [sys.executable, str(ROOT / "governance/03_校验" / "validate_run.py"), str(target), "--no-write"],
                cwd=ROOT,
                check=False,
                capture_output=True,
                text=True,
    ...
    def test_valuation_sensitivity_is_allowed_but_target_price_is_blocked(self) -> None:
        delivery = next((ROOT / "instances/01_正式样例/02_管制与国产替代").glob("05-*主题深度研究-*.md"))
