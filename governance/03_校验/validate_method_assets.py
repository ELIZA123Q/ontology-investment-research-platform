...
ASSET_PATH = ROOT / "methods/00_登记/method_assets.yaml"
STRUCTURE_PATH = ROOT / "methods/02_判断结构/00_framework_dependency_registry.yaml"
EVIDENCE_PATH = ROOT / "methods/03_取证/03_registry.yaml"
ROUTE_PATH = ROOT / "governance/02_合同/judgment_method_routes.yaml"
...
def build_method_catalog(
    ...
    for local_id, item in (evidence.get("methods") or {}).items():
        ...
        register(method_id, {
            ...
            "file": f"methods/03_取证/{item.get('file', '')}",
    ...
    for item in adjudication_group.get("methods") or []:
        ...
        register(method_id, {
            ...
            "source_ref": f"methods/00_登记/method_assets.yaml#groups.adjudication.methods.{method_id}",
            "file": item.get("file"),
