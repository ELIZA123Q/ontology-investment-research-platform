...
_SCAN_ALLOWLIST_SUFFIXES = {
    "governance/03_校验/status_derivation.py",  # 仅从本体规则加载集合
    "governance/03_校验/validate_parameter_authority.py",
    "runtime/engine/ontology_instance_graph.py",  # 兼容展开旧 level_requirements
}
...
def _scan_duplicate_authority(errors: list[str]) -> None:
    ...
    for root in scan_roots:
        ...
        for path in root.rglob("*"):
            ...
            if "/tests/" in f"/{relative}/" or relative.startswith("governance/03_校验/tests/"):
                continue
...
def main() -> int:
    ...
    domain_dir = ROOT / "ontology/02_领域/semiconductor"
    ...
    matrix_path = ROOT / "governance" / "02_合同" / "parameter_authority_matrix.yaml"
    ...
    kb03 = yaml.safe_load((ROOT / "methods/03_取证" / "03_registry.yaml").read_text(encoding="utf-8")) or {}
    if "ontology_authority_refs" not in kb03:
        errors.append("methods/03_取证/03_registry.yaml 缺少 ontology_authority_refs")
