...
from _path_setup import ensure_all_validator_paths

ensure_all_validator_paths()

from quality_gate_utils import RETURN_STAGES  # noqa: E402
...
from validate_01_outputs import validate as validate_01  # noqa: E402
from validate_02_outputs import validate as validate_02  # noqa: E402
...
from validate_05_outputs import ARCHETYPE_NAME_MAP, validate as validate_05  # noqa: E402
...
VALIDATOR_ERROR_ROUTES: dict[str, list[ErrorRoute]] = {
    "02": [
        ErrorRoute(re.compile(r"source_document|01\.|投研需求说明|stage_status"), "01", "02.validator.upstream_01", "先修正 01 需求说明，再重建 02"),
        ErrorRoute(re.compile(r"judgment_spine|minimum_question_tree|validation\.checks|state_variable_chain"), "02", "02.validator.structure", "补齐核心待验证命题、问题树、路径或状态变量链"),
    ...
    "04": [
        ErrorRoute(re.compile(r"超过.*03.*判断上限|超过.*矩阵|evidence_readiness"), "03", "04.validator.evidence_ceiling", "观点超过统一矩阵计算的判断上限，退回 03 补证据或降低 J 等级"),
        ...
        ErrorRoute(re.compile(r"path_results|state_variable_results|ontology_context|judgment_unit"), "02", "04.validator.structure", "核心待验证命题、路径或本体承接不足，退回 02"),
        ErrorRoute(re.compile(r"source_01|scope|研究范围|问题偷换"), "01", "04.validator.scope", "研究问题、用途或范围不清，退回 01"),
        ErrorRoute(re.compile(r"handoff_to_05|brief_quality_check|表达许可|answer_first"), "04", "04.validator.expression", "在 04 重写判断简报或表达许可，不放大判断"),
    ],
    "05": [
...
class RunArtifacts:
    ...
    def triplet(self) -> tuple[str, str, str] | None:
        ...
        if self.report is not None:
            return parse_triplet(self.report, "判断简报", "04")
...
def discover_artifacts(run_dir: str | Path) -> RunArtifacts:
    ...
    report = _pick_optional(list(run_dir.glob("04-*判断简报-*.md")), "04 判断简报", run_dir)
...
def _gate_01_to_02(artifacts: RunArtifacts, items: list[ReworkItem]) -> list[str]:
    ...
    if req_meta.get("stage_status") != "complete":
        errors.append("01 stage_status 必须为 complete 才能进入 02")
        _append_rework(items, rule_id="01.stage_status", message=errors[-1], return_to="01", affected_stage="02")

    if req_meta.get("task_disposition") != "accepted":
        errors.append("01 task_disposition 必须为 accepted 才能进入 02")
        _append_rework(items, rule_id="01.task_disposition", message=errors[-1], return_to="01", affected_stage="02")
    ...
    if logic_meta.get("stage_status") != "complete":
        errors.append("02 stage_status 必须为 complete 才能继续")
    ...
    if quality_control.get("stage_status") != "complete":
        errors.append("02 本体视图 stage_status 必须为 complete")
...
def _gate_02_to_03(artifacts: RunArtifacts, items: list[ReworkItem]) -> list[str]:
    ...
    if logic_meta.get("stage_status") != "complete":
        errors.append("02 stage_status 必须为 complete 才能进入 03")
...
def _gate_03_snapshot_semantic(artifacts: RunArtifacts, items: list[ReworkItem]) -> list[str]:
    ...
    for label, left, right in [
        ...
        ("manifest.stage_status", manifest.get("stage_status"), prep_meta.get("stage_status")),
...
def _gate_04_audit_semantic(artifacts: RunArtifacts, items: list[ReworkItem]) -> list[str]:
    ...
    quality_check = audit.get("brief_quality_check", {})
    if isinstance(quality_check, dict):
        errors.extend(
            _check_return_block(
                ...
                return_reason="04 brief_quality_check 要求返工",
            )
        )
        if quality_check.get("result") == "fail":
            ...
            message = reason_text or "04 brief_quality_check.result=fail"
            errors.append(message)
            _append_rework(
                items,
                rule_id="04.brief_quality_check.fail",
                message=message,
                return_to=target,
                affected_stage="04",
                fix_hint="按 brief_quality_check.return_reasons 在 04 修订判断定稿与表达许可",
            )
        bool_checks = {
...
def _gate_05_snapshot_material(artifacts: RunArtifacts, items: list[ReworkItem]) -> list[str]:
    ...
    try:
        material_rows = _snapshot_csv(artifacts.snapshot_dir, "delivery_readiness.csv")
    except Exception:
        material_rows = []
    ...
    blocking_rows = [
        ...
        if str(row.get("status", "")).strip().lower() != DELIVERY_READY_STATUS
        and str(row.get("target_report_type", "")).strip() in {"", archetype}
    ]
    for row in blocking_rows:
        required_action = str(row.get("gap_action", "")).strip()
        return_target = _parse_return_action(required_action) or "03"
        message = (
            f"目标报告 {row.get('target_report_type', '?')} 的 delivery_readiness 未就绪"
            f"（status={row.get('status', '')}；limit={row.get('delivery_limit', '')}）"
        )
        if required_action:
        ...
        _append_rework(
            items,
            rule_id=f"05.delivery.{row.get('delivery_readiness_id', 'unknown')}",
            message=message,
            return_to=return_target,
            affected_stage="05",
            fix_hint=required_action or "按 delivery_limit 降级交付，或退回 03 补齐目标报告实际需要的素材",
        )
    ...
    try:
        ...
        gap_rows = []
    for row in gap_rows:
        if str(row.get("gap_type", "")).strip() != "delivery_material":
            continue
    ...
    if artifacts.delivery is not None:
        ...
        applicable_rows = [
            row
            for row in material_rows
            if str(row.get("target_report_type", "")).strip() in {"", archetype}
            and str(row.get("status", "")).strip() == DELIVERY_READY_STATUS
        ]
...
def _gate_03_to_04(artifacts: RunArtifacts, items: list[ReworkItem]) -> list[str]:
    ...
    refs = [
        ...
        ("audit.brief_ref", metadata.get("brief_ref"), artifacts.report),
    ]
...
def _validate_stage_chain(
    ...
    *,
    ...
    if upstream_ok:
        ...
        report_triplet = parse_triplet(artifacts.report, "判断简报", "04")
    ...
    if low_quality_stages and through == "05" and publish_status != "INCOMPLETE_CHAIN":
        for stage in low_quality_stages:
            _append_rework(
                ...
                return_to=stage,
                affected_stage=stage,
                fix_hint="按阶段规范修正并达到 high_quality_pass",
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--report", help="04 判断简报路径")
