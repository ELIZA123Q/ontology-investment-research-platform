...
WORKSPACE = ROOT.parents[2]  # 03_capabilities/05_method_libraries/<lib> → repo
...
EVIDENCE_GUIDE_ROOT = WORKSPACE / "01_semantic/04_evidence"
MCP_GUIDE_ROOT = WORKSPACE / "03_capabilities/04_protocols/mcp"
    ...
    for group_name, refs in references.items():
        ...
        for ref in refs:
            ...
            resolved = candidate if candidate.is_absolute() else (
                WORKSPACE / candidate if len(candidate.parts) > 1 else ROOT / candidate
            )
...
def validate_frontstage(errors: list[str]) -> None:
    ...
    b_files = sorted(p.name for p in EVIDENCE_GUIDE_ROOT.glob("B0*.md"))
    required_b = {
        "B00_来源选择与使用边界.md",
        "B01_通用来源速查.md",
        "B02_半导体来源速查.md",
    ...
    b00 = EVIDENCE_GUIDE_ROOT / "B00_来源选择与使用边界.md"
    if b00.is_file():
...
def validate_ops_and_qp_links(errors: list[str]) -> None:
    ...
    for guide_name in ("B01_通用来源速查.md", "B02_半导体来源速查.md"):
        ...
        if not refs:
            errors.append(f"{guide_name} 应至少链接若干 OPS QP ID，避免与 OPS 平行维护")
        for qp in refs:
            if qp not in all_qps:
                errors.append(f"{guide_name} 引用了不存在的 QP: {qp}")
