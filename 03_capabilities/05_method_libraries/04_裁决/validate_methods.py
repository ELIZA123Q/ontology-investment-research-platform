...
PROJECT_ROOT = ROOT.parents[2]  # 03_capabilities/05_method_libraries/04_裁决 → repo
...
BOUNDARY_FILES = {
    "03_capabilities/05_method_libraries/02_研究框架/README.md": ["03_capabilities/05_method_libraries/03_取证", "03_capabilities/05_method_libraries/04_裁决"],
    "03_capabilities/05_method_libraries/03_取证/README.md": ["03_capabilities/05_method_libraries/02_研究框架", "03_capabilities/05_method_libraries/04_裁决"],
    "02_tasks/04_workflows/deep_research/stages/04_judgment.md": [
        "03_capabilities/05_method_libraries/04_裁决/README.md",
        "不新增判断路径",
        "四份附录",
        "附录4",
    ],
    "05_governance/01_架构/00_项目定位与边界.md": [
        "02 声明可以怎样组织判断",
        "03 声明什么证据可以用",
        "04 声明证据可以怎样裁决",
    ],
...
def check_required_assets(errors: list[str]) -> None:
    for relative_path, required_phrases in REQUIRED_FILES.items():
        ...
        if not path.exists():
            errors.append(f"缺少必需文件：03_capabilities/05_method_libraries/04_裁决/{relative_path}")
            continue
...
def check_path_result_status_contract(errors: list[str]) -> None:
    ...
    output_spec = (PROJECT_ROOT / "02_tasks/04_workflows/deep_research/stages/04_judgment.md").read_text(
        encoding="utf-8"
    )
    missing_in_spec = PATH_RESULT_STATUS_CODES - table_status_codes(output_spec)
    if missing_in_spec:
        errors.append(f"02_tasks/04_workflows/deep_research/stages/04_judgment.md 缺少路径状态码：{sorted(missing_in_spec)}")
