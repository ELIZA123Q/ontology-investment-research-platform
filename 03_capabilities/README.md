...
**重要：** 本域主要是**治理索引与方法正文**。真正会被代码加载的 Agent/Skill/Tool 清单只在 [`07_runtime/src/capabilities/registry.ts`](../07_runtime/src/capabilities/registry.ts)。
...
- 改可执行能力：必须改 `07_runtime/src/capabilities/registry.ts`，并跑 Runtime 测试与 `audit-cutover`；本域只同步治理归属，不存实现镜像。
