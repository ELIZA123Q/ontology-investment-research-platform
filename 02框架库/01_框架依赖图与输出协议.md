...
02 的基础框架按研究链条分为三层。先生成一个明确的判断单元，再沿依赖关系逐段放行；框架标题与题目关键词相同，不代表该框架已经可以调用。
...
半导体 8 个行业主框架是三层中的专业化覆盖层，不另建一条平行链：IF-SC、IF-LOC、IF-APP、IF-FAB、IF-PKG 属于基础机制层；IF-DES、IF-EQP、IF-MAT 跨入公司兑现层。它们同样必须声明 Framework Output Contract，并受基础框架前置门禁约束。
...
02 交给 03 的不是框架全文，而是上述合同中的 CandidateClaim、变量和信号候选、证据要求、反证、场景与未决缺口。03 为这些候选建立现实实例、观测和证据，并返回可用性；04 只对已具备证据状态的 JudgmentUnit 做逐段放行、竞争解释裁决和改判。
