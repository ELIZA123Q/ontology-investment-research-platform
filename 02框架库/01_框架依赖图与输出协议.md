...
[`00_framework_dependency_registry.yaml`](00_framework_dependency_registry.yaml) 是框架层级、进入条件、输出门槛、质量门槛、下游解锁和职责交接的**唯一机器事实源**。框架 front matter 不得出现 `builds_on`；正文合同不得重新声明 `hard_prerequisites`、`quality_gate` 或静态 `downstream_unlocks`。
...
| `output_gates` | 现有前置和最低证据允许输出哪一种 CandidateClaim | 只禁止该级输出；较低级输出仍可保留 |
...
这不是必须完整执行的流水线。任务可以停在任一输出门槛；只有要生成更高层结论时，才加载该门槛明确要求的上游输出。

```mermaid
flowchart TD
    J["判断类型 + 原子 CandidateClaim"] --> M["基础或行业机制输出"]
    M --> C["公司暴露与兑现输出"]
    C --> F["条件预测与情景输出"]
    F --> E["事前预期差输出"]
    E --> V["估值输入与区间输出"]
...
图只表达默认研究方向，不承担机器依赖定义。精确门槛始终读取 registry。
...
框架调用仍保留统一 `gate_status`，但它只表示“本次调用及所选输出中最弱门槛”的状态，不表示整个框架已经成立。

| 状态 | 含义 | 下游动作 |
|---|---|---|
| `passed` | 本次实际使用的输出门槛全部满足 | 可按 registry 考虑相应下游 |
...
每个正式框架声明自己能生成哪些候选对象，并用 `output_gate_refs` 引用 registry 中的门槛。静态合同不再复制依赖关系。
...
`downstream_unlocks` 是本次实际通过门槛后，从 registry 解析得到的运行结果，不是框架正文的静态清单。兼容旧任务时，`candidate_claims` 可暂时使用字符串；新任务优先使用上面的原子命题结构。
...
| 02：判断单元生成与路由 | 03：证据准备 | 04：裁决 |
|---|---|---|
| 选择判断类型，生成原子 CandidateClaim | 把候选变量、信号和最低证据变成可取证任务 | 判断证据是否支持、削弱、争议或阻断 |
...
- 来源分层、证据组合和获取渠道由 [`03取证策略库`](../03取证策略库/README.md) 管理。框架只写与特有输出直接相关的最低证据，不复制通用来源类型。
