...
| 02 | 02-主题研究逻辑-日期-序号.md | 02-主题本体视图-日期-序号.yaml | 研究逻辑 + 本次任务的跨三域本次需要的本体范围 |
| 03 | 03-主题数据与证据准备-日期-序号.md | 03-主题语义域与证据域实例清单-日期-序号.yaml + 快照目录 | 语义实例、证据实例、固定推理输入和证据准入 |
...
02 本体视图必须同时包含 semantic_scope、evidence_contract 和 reasoning_plan。02 只引用正式本体，不复制或改写正式定义。03、04 产生本次任务记录；任何新能力只能进入候选与缺口文件，不能由单次任务直接写回正式本体。
