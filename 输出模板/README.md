...
- `05_模板索引.md` 说明 05A-05E 输出原型模板；`05_研报完整内容输出模板.md` 仅保留为历史兼容入口，不再作为唯一生产模板。
- 正式文件命名为 `05-<核心主题><输出原型名称>-<YYYYMMDD>-<当日序号>.md`，例如 `05-存储芯片周期行业周期判断-20260707-1.md`。
...
- `validate_05_outputs.py` 用于检查 05 稿件是否匹配 01 `delivery_archetype.primary`，并校验必备章节、正文纯净度、预期差来源、强弱排序、图表数据密度、判断强度和合规边界。
