...
先形成项目直觉，不要从本体 YAML、治理合同或 Runtime 代码开始：

1. 读本页到“仓库怎么找东西”，理解项目解决什么问题、五阶段怎样分工。
2. 读一份成品：[`存储周期研报`](instances/02_V3样例/01_memory-cycle-run-002/05_report.md)。
3. 再看它背后的 [`判断产物`](instances/02_V3样例/01_memory-cycle-run-002/04_judgment.yaml)，理解“结论—依据—边界—改判条件”怎样落盘。
4. 真正开始研究时，按 [`workflow/`](workflow) 的 01—05 推进；需要具体方法时再进入 [`methods/`](methods)。

暂时可以跳过：`ontology/`、`governance/02_合同/`、`runtime/engine/`、`evaluation/`。它们分别服务于语义维护、机器合同、产品开发和正式评测，不是理解项目的前置阅读。

完整的目录职责、文件保留规则和重复文件解释见
[`仓库地图与文件治理`](governance/01_架构/02_仓库地图与文件治理.md)。
