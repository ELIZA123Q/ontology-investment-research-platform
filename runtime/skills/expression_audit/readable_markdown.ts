...
import { STAGE01_QUALITY_GATE_REF } from "../../agents/01_intake/input_contract";
import { ensureStage02DocumentFields, STAGE02_QUALITY_GATE_REF } from "../../agents/02_structure/input_contract";
import { ensureStage03DocumentFields, STAGE03_QUALITY_GATE_REF } from "../../agents/03_evidence/input_contract";
import { ensureStage04DocumentFields, STAGE04_QUALITY_GATE_REF } from "../../agents/04_judgment/input_contract";
...
export function syncStage01ReadableMarkdown(data: any, question: string): string {
  ...
  data.document_markdown = [
    yamlFrontmatter({
      ...
      schema_version: data.schema_version || "1.5.0",
      task_id: data.task_id || "",
      requirement_version: data.requirement_version || "1.0.0",
      ...
      deterministic_check_status: data.deterministic_check_status || "not_checked",
      semantic_review_status: data.semantic_review_status || "not_reviewed",
      return_required: data.return_required ?? false,
      return_stage: data.return_stage ?? null,
      original_input: data.original_input || question,
      normalized_question: data.normalized_question || "",
      judgment_landing: data.judgment_landing || "",
      task_type: data.task_type || { primary: "state_assessment", secondary: [] },
      delivery_archetype: data.delivery_archetype || { primary: "状态定位型", secondary: [], modules: [] },
      intended_use: data.intended_use || ["研究参考"],
      not_allowed_use: data.not_allowed_use || ["trading_recommendation"],
      scope_summary: data.scope_summary || "",
      main_judgment_axis: data.main_judgment_axis || {},
      research_value_gate: data.research_value_gate || {},
      overscope_check: data.overscope_check || {},
      needs_split: data.needs_split ?? false,
      input_resolution: data.input_resolution || {},
      time_scope: data.time_scope || {},
      delivery_depth: data.delivery_depth || {},
      task_scope_contract: data.task_scope_contract
        || { root_scope_ref: "root", required_split_scope_refs: ["root"], comparison_policy: "allow_unified_if_supported", prohibited_aggregation_outcomes: [] },
    }),
    "# 投研需求说明",
    "",
    "## 1. 原始输入与解析",
...
export function syncStage03ReadableMarkdown(
  ...
  if (!options.forceProjection && shouldPreserveStage03Markdown(existing)) {
  ...
  const prep = [
    yamlFrontmatter({
      ...
      schema_version: data.schema_version || "3.0.0",
      task_id: data.task_id || options.taskId || "",
      execution_id: data.execution_id || `EXEC-${options.taskId || ""}`,
      plan_id: data.plan_id || "",
      source_02_view_id: data.source_02_view_id || "",
      source_02_logic_id: data.source_02_logic_id || "",
      source_02_view_ref: data.source_02_view_ref || "02-本体视图.yaml",
      source_02_logic_ref: data.source_02_logic_ref || "02-研究逻辑.md",
      execution_date: data.execution_date || new Date().toISOString(),
      timezone: data.timezone || "Asia/Shanghai",
      data_cutoff: data.data_cutoff || "",
      resolved_anchor_date: data.resolved_anchor_date || "",
      resolved_start: data.resolved_start || "",
      resolved_end: data.resolved_end || "",
      resolved_at: data.resolved_at || "",
      resolved_objects: data.resolved_objects || "",
      target_05_archetype: data.target_05_archetype || "状态定位型",
      target_05_quality: data.target_05_quality || "high_quality_pass",
      source_registry_version: data.source_registry_version || "1.0.0",
      recipe_library_version: data.recipe_library_version || "1.0.0",
      stage_status: data.stage_status,
      quality_status: data.quality_status,
      ...
      coverage_unit_total: data.coverage_unit_total ?? 0,
      evidence_backed_unit_count: data.evidence_backed_unit_count ?? 0,
      evidence_coverage_rate: data.evidence_coverage_rate,
      required_coverage_rate: data.required_coverage_rate ?? 0.5,
      critical_node_gate_status: data.critical_node_gate_status,
      judgment_unit_gate_status: data.judgment_unit_gate_status,
      search_status: data.search_status,
      snapshot_ref: data.snapshot_ref,
      snapshot_summary_ref: data.snapshot_summary_ref || "03-证据快照摘要.yaml",
