...
export function loadMethodRegistry(): Map<string, RegisteredMethod> {
  const root = parseYaml("governance/knowledge_changes/method_assets.yaml") as MethodAssetRegistry;
  ...
  for (const method of adjudication?.methods || []) {
    result.set(method.id, {
      ...
      source_ref: `governance/knowledge_changes/method_assets.yaml#groups.adjudication.methods.${method.id}`,
