...
def _view() -> dict:
    path = next((ROOT / "instances/01_正式样例/01_存储周期").glob("02-*本体视图-*.yaml"))
...
class ResearchLoopTests(unittest.TestCase):
    def test_current_02_contract_survives_instance_graph_projection(self) -> None:
        view_source = next((ROOT / "instances/01_正式样例/01_存储周期").glob("02-*本体视图-*.yaml"))
        ...
        view["iteration_contract"] = {
            ...
            "evidence_wave_schema_ref": "runtime/engine/templates/evidence_wave.template.yaml",
            "dependency_projection_schema_ref": "runtime/engine/templates/dependency_projection.template.yaml",
            ...
            "convergence_contract_ref": "governance/02_合同/public_contract.yaml#iteration_semantics",
        }
    ...
    def test_manifest_false_convergence_blocks_release(self) -> None:
        artifacts = discover_artifacts(ROOT / "instances/01_正式样例/01_存储周期")
    ...
    def test_manifest_creates_pending_attempt_without_overwriting_current(self) -> None:
        ...
        manifest = yaml.safe_load((ROOT / "instances/01_正式样例/01_存储周期" / "run_manifest.yaml").read_text(encoding="utf-8"))
