...
export const judgmentDecisionSchema = z.object({
  ...
  business_impacts: z.array(z.object({
    id: nonEmptyString,
    statement: nonEmptyString,
    source_judgment_refs: z.array(z.string()).min(1),
    target_object_ref: nonEmptyString,
    impact_driver: z.enum(ONTOLOGY_ENUMS["BusinessImpact.impact_driver"]),
    direction: z.enum(ONTOLOGY_ENUMS["BusinessImpact.direction"]),
    time_horizon: nonEmptyString,
  ...
  financial_impacts: z.array(z.object({
    id: nonEmptyString,
    statement: nonEmptyString,
    financial_metric: z.enum(ONTOLOGY_ENUMS["FinancialImpact.financial_metric"]),
    direction: z.enum(ONTOLOGY_ENUMS["FinancialImpact.direction"]),
    time_horizon: nonEmptyString,
    basis_refs: z.array(z.string()).min(1),
    business_impact_refs: z.array(z.string()).default([]),
    conditions: z.array(z.string()).min(1),
  })).default([]),
  estimate_revisions: z.array(z.object({
    id: nonEmptyString,
    statement: nonEmptyString,
    revision_type: z.enum(ONTOLOGY_ENUMS["EstimateRevision.revision_type"]),
    direction: z.enum(ONTOLOGY_ENUMS["EstimateRevision.direction"]),
    horizon: nonEmptyString,
    prior_value: z.string().nullable().optional(),
    revised_value: z.string().nullable().optional(),
    basis_refs: z.array(z.string()).min(1),
    financial_impact_refs: z.array(z.string()).default([]),
  })).default([]),
  asset_impacts: z.array(z.object({
    ...
    conditions: z.array(z.string()).min(1),
    financial_impact_refs: z.array(z.string()).default([]),
    estimate_revision_refs: z.array(z.string()).default([]),
    limitations: z.array(z.string()).default([]),
  })).default([]),
