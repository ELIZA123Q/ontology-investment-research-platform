...
**已实现。** 核心编排逻辑位于 `runtime/workflow/controller.ts`。
...
| 文件 | 用途 |
|------|------|
| `governance/contracts/governance_control_contract.yaml` | 治理控制合同 |
| `runtime/workflow/controller.ts` | 核心编排实现 |
| `runtime/app/api/runs/[id]/pipeline/route.ts` | API 端点 |
| `runtime/workflow/approval.ts` | 确认管线 |
| `runtime/runner/research_job_runner.ts` | 任务入队与执行 |
