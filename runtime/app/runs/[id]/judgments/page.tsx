...
export default async function Judgments({ params }: { params: Promise<{ id: string }> }) {
  ...
  competitors.forEach((item: any, index: number) => {
    nodes.push({ id: item._id, label: item.statement, meta: "竞争解释", tone: item.status === "eliminated" ? "unknown" : "weaken", x: 650, y: Math.max(220, hypotheses.length * 155) + index * 130, details: { 状态: item.status || "未排除", 信号: item.signal_ids || item.signal_refs || [], 排除理由: item.elimination_rationale || "" } });
  ...
  judgments.forEach((item: any, index: number) => {
    ...
    for (const competitor of competitors) edges.push({ id: `${competitor._id}-${item._id}`, source: competitor._id, target: item._id, tone: "weaken", label: "竞争解释" });
  });
