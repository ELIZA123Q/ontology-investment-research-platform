...
export default async function HistoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    <RunNav runId={id} active="history" />
    <div className="pagehead scene-head"><div><div className="eyebrow">Research history</div><h1>这次判断是怎样变化的？</h1><p className="muted">父子运行、触发事件和阶段版本均保留，不覆盖历史结论。</p></div></div>
    <div className="history-grid">
      <section className="card"><div className="panel-title"><div><span>运行关系</span></div></div>{parent ? <Link className="history-run parent" href={`/runs/${parent.id}`}><span>父运行</span><strong>{parent.question}</strong><small>{parent.created_at}</small></Link> : <div className="history-run root"><span>根运行</span><strong>这是该研究链的起点</strong></div>}{children.map((child) => <Link className="history-run child" href={`/runs/${child.id}`} key={child.id}><span>增量运行</span><strong>{child.question}</strong><small>{child.created_at}</small></Link>)}</section>
      <section className="card"><div className="panel-title"><div><span>阶段版本</span><strong>{artifacts.length}</strong></div></div><div className="artifact-ledger">{artifacts.map((artifact) => <div key={artifact.id}><span>{artifact.kind}</span><strong>v{artifact.version} · {artifact.status}</strong><small>{artifact.created_at}</small></div>)}</div></section>
      {comparisons.map(({ previous, current, attribution }) => <section className="card history-comparison" key={`${previous.id}-${current.id}`}>
        <div className="panel-title"><div><span>父子运行差异</span><strong>{classificationLabel(current.trigger_classification)}</strong></div><Link href={`/runs/${current.id}`}>查看子运行 →</Link></div>
        <div className="comparison-head"><div><small>父运行</small><strong>{previous.id.slice(0, 8)}</strong></div><span>→</span><div><small>子运行</small><strong>{current.id.slice(0, 8)}</strong></div></div>
