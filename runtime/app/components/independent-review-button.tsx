...
export function IndependentReviewButton({ runId, completed = false }: { runId: string; completed?: boolean }) {
  ...
  async function submitHumanReview() {
    ...
    if (!reviewer.trim() || attestation.trim().length < 20 || assessment.trim().length < 8 || selectedIssues.some((issue) => !issue.description || !issue.required_action)) {
      setError("请填写审阅者、至少 20 字独立性声明、总体意见；rework 还必须填写每项问题和返工动作。");
      return;
    }
    setBusy(true);
    setError("");
  ...
  return <div className="independent-review-actions">
    ...
    {open && !completed ? <div className="card human-review-form">
      <p className="muted">必须由未参与 Stage04 生产的人类独立核查。系统冻结被审阅产物 hash，并保留审阅者标识和利益冲突声明；这不是“自己点一下通过”。</p>
      <div className="source-form-grid">
        <div className="field"><label>审阅者标识</label><input value={reviewer} onChange={(event) => setReviewer(event.target.value)} /></div>
        <div className="field"><label>审阅结论</label><select value={verdict} onChange={(event) => setVerdict(event.target.value as "pass" | "rework")}><option value="pass">pass</option><option value="rework">rework</option></select></div>
      ...
      {verdict === "rework" ? <div>{issues.map((issue, index) => <div className="card" key={index} style={{ marginTop: 10 }}>
        ...
        <div className="source-form-grid">
          ...
          <div className="field"><label>退回阶段</label><select value={issue.return_stage} onChange={(event) => updateIssue(index, { return_stage: event.target.value as ReturnStage })}><option value="stage_02">Stage02</option><option value="stage_03">Stage03</option><option value="stage_04">Stage04</option></select></div>
          <div className="field"><label>Judgment ID（可空）</label><input value={issue.judgment_id} onChange={(event) => updateIssue(index, { judgment_id: event.target.value })} /></div>
          <div className="field"><label>Evidence refs（逗号分隔）</label><input value={issue.evidence_refs} onChange={(event) => updateIssue(index, { evidence_refs: event.target.value })} /></div>
