...
export function researcherLanguage(value: unknown): string {
  return stripInternalReferencePrefix(value)
    ...
    .replace(/\bGAP-[A-Za-z0-9._-]+\b/gi, "证据缺口")
    ...
    .replace(/标记为\s*gap/gi, "登记为证据缺口")
    .replace(/\bgap\b/gi, "证据缺口")
    .replace(/本次本次自动检索/g, "本次自动检索")
    .replace(/本次证据收集阶段面临本次自动检索/g, "本次证据收集受自动检索")
    .replace(/标记为缺口[（(]证据缺口[）)]/g, "登记为证据缺口")
    ...
    .replace(/上游只有经人工接受的证据缺口/g, "上游目前只有研究员确认的证据缺口")
    .replace(/经人工接受的证据缺口/g, "研究员确认的证据缺口")
