...
def mock_artifact(case_id: str, kind: str, target_length: int) -> str:
    ...
    headings = {
        "question_only": "普通问题直答稿",
        "same_evidence_direct": "同证据直接生成报告",
        "summary": "同证据条件摘要稿",
    }
    heading = headings.get(kind, "普通生成稿")
    body = [f"# {heading}", ""]
...
def build_result(request: dict[str, Any], profile: str) -> dict[str, Any]:
    ...
    if role == "question_only_generator":
        text = mock_artifact(case_id, "question_only", int(payload.get("target_length") or 900))
        return {"artifact_text": text, "target_length_followed": True}

    if role == "same_evidence_direct_generator":
        text = mock_artifact(case_id, "same_evidence_direct", int(payload.get("target_length") or 900))
        return {"artifact_text": text, "target_length_followed": True}

    if role == "evidence_summary_generator":
        text = mock_artifact(case_id, "summary", int(payload.get("target_length") or 900))
        return {"artifact_text": text, "target_length_followed": True}
    ...
    if role == "downstream_scorer":
        ...
        base_scores = {"A": 0.56, "B": 0.70, "C": 0.80, "D": 0.89, "E": 0.93}
        adjustment = {
    ...
    if role == "pairwise":
        ...
        if baseline == "same_evidence_direct":
            # 同证据直出更接近系统产物，胜率仍为正但更紧。
            normalized = ["D", "D", "D", "tie", "tie", "baseline"][run_index]
        else:
