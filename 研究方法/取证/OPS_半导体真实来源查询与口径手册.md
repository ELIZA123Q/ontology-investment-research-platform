...
### A股半导体可复现样本边界（QP-SEMI-10）

| 项目 | 操作要求 |
|---|---|
| 正式入口 | [中证半导体产业指数 931865](https://www.csindex.com.cn/#/indices/family/detail?indexCode=931865)、[编制方案](https://oss-ch.csindex.com.cn/static/html/csindex/public/uploads/indices/detail/files/zh_CN/931865_Index_Methodology_cn.pdf)、指数事实表与样本／权重下载 |
| 查询方法 | 先固定信息截面，再取得当时生效的样本名单、权重和调样公告；分别记录文件日期、样本生效日和指数代码。动态页面显示的当前样本不得回填历史截面 |
| 核心口径 | 931865 是按可投资性和指数规则选出的不超过40只证券，用作可复现的A股半导体市场代理；不是所有A股半导体公司的无偏总体，也不是产业收入或产量总体 |
| 取得后记下 | 指数代码、编制方案版本、样本生效日、事实表日期、样本证券、权重、取得时间和文件URL |
| 同源回退 | 官方样本下载 → 官方调样公告 → 官方事实表；不同文件日期不一致时保留各自适用时点 |
| 跨源回退 | 官方动态页面通道失败 → `akshare_mcp` 发现候选样本，再回到中证公告核对；无法回到官方时只能形成受限代理并登记 `trace_missing` |
| 使用边界 | 指数样本统计只能表述为“931865样本口径”；产业链外补公司只做机制或覆盖补充，不并入指数聚合值 |

若研究目标是全部A股半导体公司，应另行定义完整纳入／排除规则并冻结证券列表，不能把931865自动改名为“全行业”。
...
| A股样本边界 | 中证931865历史生效样本 | 官方调样公告／事实表 | 自定义冻结样本并披露规则 | 聚合接口候选成分 | 指数样本结论不得外推全部A股半导体公司 |
