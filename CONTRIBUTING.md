...
新增/移除 MCP 数据通道时，同步更新上述文件 + `03_agent_capability/04_protocols/mcp/ops/B03_MCP通道注册.md` + `OPS_MCP查询快速参考.md` + `03_registry.yaml`。

## 必过检查

```bash
python3 05_control_evaluation/04_verifiers/validate_project.py
python3 05_control_evaluation/04_verifiers/validate_v3_samples.py
npm --prefix 06_runtime run typecheck
npm --prefix 06_runtime test
npm --prefix 06_runtime run build
```

可选本地检查：

```bash
npm --prefix 06_runtime run lint
python3 -m pip install ruff && ruff check 05_control_evaluation/04_verifiers
```
...
- 公共字段、ID、跨阶段引用先改 `05_control_evaluation/01_rules/contracts/public_contract.yaml`，再同步 Runtime、校验器和样例。
