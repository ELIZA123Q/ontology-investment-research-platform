...
PROJECT_ROOT = ROOT.parents[3]  # 90_compat/methods/04_裁决 → repo
...
BOUNDARY_FILES = {
    "90_compat/methods/02_判断结构/README.md": ["90_compat/methods/03_取证", "90_compat/methods/04_裁决"],
    "90_compat/methods/03_取证/README.md": ["90_compat/methods/02_判断结构", "90_compat/methods/04_裁决"],
...
def check_required_assets(errors: list[str]) -> None:
    for relative_path, required_phrases in REQUIRED_FILES.items():
        ...
        if not path.exists():
            errors.append(f"缺少必需文件：90_compat/methods/04_裁决/{relative_path}")
            continue
