...
依据 [03附录2 第5节](../../runtime/workflow/stage_specs/03_证据/03_附录2_取数留痕与材料处理操作手册.md)，每次通过 MCP 获取的材料必须记录：
...
18 个证据通道的机器权威清单为 [`mcp_channels.yaml`](mcp_channels.yaml)；它必须与 `governance/02_合同/ontology_data_mapping_profiles.yaml` 的 `required_connectors` 和 active Profile 一致。MCP 故障时 Stage03 按通道回退提示转向可核验原文或公开网页，并诚实登记 gap。
