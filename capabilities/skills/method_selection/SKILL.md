...
| 文件 | 职责 |
|------|------|
| `method_registry.ts` | 方法注册表加载：从 `methods/00_登记/` 加载全部已注册方法，支持按判断类型路由 |
