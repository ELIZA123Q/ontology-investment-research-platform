...
通道到来源类别的映射见取证方法库 [B01 通用来源速查](../../05_method_libraries/03_取证/B01_通用来源速查.md) 和 [B02 半导体来源速查](../../05_method_libraries/03_取证/domains/semiconductor/B02_半导体来源速查.md)。本文件只维护通道本身，不在来源速查表里反向绑定 Tool 名。
...
每个 MCP 作为获取通道，映射到 [B00 来源角色](../../05_method_libraries/03_取证/B00_来源选择与使用边界.md)：
...
依据 [03附录2 第5节](../../02_scenario_task/04_workflows/deep_research/supporting/03_evidence_provenance_manual.md)，每次通过 MCP 获取的材料必须记录：
...
18 个证据通道的机器权威清单为 [`mcp_channels.yaml`](mcp_channels.yaml)；它必须与 `05_control_evaluation/02_合同/ontology_data_mapping_profiles.yaml` 的 `required_connectors` 和 active Profile 一致。MCP 故障时 Stage03 按通道回退提示转向可核验原文或公开网页，并诚实登记 gap。
