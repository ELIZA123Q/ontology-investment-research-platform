...
PROJECT_ROOT = ROOT.parents[2]  # 03_agent_capability/02_skills/<package> → repo
...
BOUNDARY_FILES = {
    "03_agent_capability/02_skills/research_design/references/README.md": [
        "evidence-research",
        "judgment-reasoning",
    ],
    "03_agent_capability/02_skills/evidence_research/references/README.md": [
        "research-design",
        "judgment-reasoning",
    ],
    "03_agent_capability/02_skills/judgment_reasoning/references/README.md": [
        "不新增判断路径",
        "四份附录",
        "附录4",
    ],
    "docs/architecture/00_项目定位与边界.md": [
        "02 声明可以怎样组织判断",
        "03 声明什么证据可以用",
        "04 声明证据可以怎样裁决",
    ],
...
def check_required_assets(errors: list[str]) -> None:
    for relative_path, required_phrases in REQUIRED_FILES.items():
        ...
        if not path.exists():
            errors.append(f"缺少必需文件：03_agent_capability/02_skills/judgment_reasoning/references/{relative_path}")
            continue
    ...
    for relative_path, required_phrases in BOUNDARY_FILES.items():
        ...
        for phrase in required_phrases:
            if phrase not in text:
                errors.append(f"{relative_path} 缺少相邻 Skill 边界说明：{phrase}")


def check_canonical_method_files(errors: list[str]) -> dict[int, Path]:
    catalog: dict[int, Path] = {}
    for number in range(1, 11):
        matches = sorted(REFERENCES.glob(f"A{number:02d}_*.md"))
        if len(matches) != 1:
...
def check_a_contracts(errors: list[str], warnings: list[str]) -> None:
    ...
    registry_text = (REFERENCES / "A00-附录4_通用认知错误与误用索引.md").read_text(
        encoding="utf-8"
    )
    ...
    a00 = (REFERENCES / "A00_裁决总则.md").read_text(encoding="utf-8")
    for phrase in (
        "不按方法编号机械封顶",
        "至少两条实质独立证据链",
...
def check_path_result_status_contract(errors: list[str]) -> None:
    appendix1 = REFERENCES / "A00-附录1_路径与阻断协议.md"
...
def check_rule_scenarios(errors: list[str]) -> None:
    ...
    a09 = (REFERENCES / "A09_预期差判断.md").read_text(encoding="utf-8")
    a09_requirements = (
    ...
    a10 = (REFERENCES / "A10_投资命题裁决.md").read_text(encoding="utf-8")
    a10_requirements = (
        "同一组敏感变量",
        "方向、幅度区间、窗口和可逆性",
        "不得写“收益风险比显著有利”",
...
def check_titles_and_sections(errors: list[str]) -> None:
    for path in sorted(REFERENCES.glob("A[0-9][0-9]_*.md")):
...
def check_boundary_language(errors: list[str]) -> None:
    paths = list(REFERENCES.glob("A[0-9][0-9]_*.md")) + list(REFERENCES.glob("A00*.md"))
