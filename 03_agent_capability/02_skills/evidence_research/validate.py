...
DOMAIN_SEMI_ROOT = REFERENCES / "domains" / "semiconductor"
...
def validate_registry(registry: dict[str, Any], errors: list[str]) -> None:
    if registry.get("registry_type") != "evidence_method_registry":
        errors.append("registry.yaml.registry_type 必须为 evidence_method_registry")
    if str(registry.get("schema_version")) != "3.2.0":
        errors.append("registry.yaml.schema_version 必须精确为 3.2.0")
    if "ontology_authority_refs" not in registry:
        errors.append("registry.yaml 缺少 ontology_authority_refs")
    ...
    for group_name, refs in references.items():
        ...
        for ref in refs:
            ...
            if candidate.is_absolute():
                ...
                root_relative = REFERENCES / candidate
...
def validate_frontstage(errors: list[str]) -> None:
    expected = [
        ROOT / "SKILL.md",
        REFERENCES / "00_取证任务卡.md",
        REFERENCES / "00_证据质量与结论上限.md",
    ]
    for path in expected:
        if not path.is_file():
            errors.append(f"缺少研究员入口文件: {path.relative_to(ROOT)}")

    quality = REFERENCES / "00_证据质量与结论上限.md"
    if quality.is_file():
    ...
    a_files = sorted(p.name for p in REFERENCES.glob("A0*.md"))
    if len(a_files) != 9:
    ...
    b_root = {p.name for p in REFERENCES.glob("B0*.md")}
    ...
    # B03 为可选的 MCP 通道注册
    b03 = MCP_GUIDE_ROOT / "ops" / "B03_MCP通道注册.md"
    if b03.is_file():
    ...
    b00 = REFERENCES / "B00_来源选择与使用边界.md"
    if b00.is_file():
...
def validate_ops_and_qp_links(errors: list[str]) -> None:
    ops_files = sorted(REFERENCES.glob("OPS_*.md")) + sorted(DOMAIN_SEMI_ROOT.glob("OPS_*.md"))
    core_ops_names = {
    ...
    qp_exempt = {"OPS_证据获取留痕手册.md"}
    for path in ops_files:
        text = path.read_text(encoding="utf-8")
        if path.name in qp_exempt:
            continue
