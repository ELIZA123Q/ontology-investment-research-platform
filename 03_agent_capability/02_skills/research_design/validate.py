...
WORKSPACE = ROOT.parents[2]  # 03_agent_capability/02_skills/<package> → repo
sys.path.insert(0, str(WORKSPACE / "05_control_evaluation/04_verifiers"))
...
def classify_asset(path: Path, root: Path, front: Mapping[str, Any]) -> str:
    try:
        ...
        parts = path.parts
    # Skill package keeps frameworks under references/
    if parts and parts[0] == "references":
        parts = parts[1:]
    name = path.name
    doc_type = str(front.get("document_type", ""))
    if name == "README.md" or name.startswith("00_") or name.startswith("_legacy") or "治理" in parts:
        return "skip"
    if doc_type in {
    ...
    if doc_type == "semiconductor_scenario_card" and path.parent.resolve() == SCENARIO_CARD_ROOT.resolve():
        return "scenario_card"
    if (parts and parts[0] == "基础") or "主框架" in parts:
        return "framework"
    if front.get("framework_id") and "行业" not in parts:
        return "framework"
    if len(parts) >= 2 and parts[:2] == ("行业", "半导体"):
        return "invalid"
    return "skip"


def iter_markdown_assets(root: Path) -> List[Path]:
    files = list((root / "references" / "基础").glob("*.md"))
    files += list((root / "references" / "行业").glob("**/*.md"))
    if SCENARIO_CARD_ROOT.is_dir():
        files += list(SCENARIO_CARD_ROOT.glob("SCN-*.md"))
    return sorted(files)
...
def ontology_identifiers(workspace: Path) -> Set[str]:
    ...
    paths: List[Path] = []
    models = workspace / "01_semantic_knowledge/01_ontology/models"
    if models.is_dir():
        paths.extend(sorted(models.glob("*.yaml")))
    domain = workspace / "01_semantic_knowledge/01_ontology/domains/semiconductor"
    if domain.is_dir():
...
def validate(root: Path) -> Tuple[List[str], List[str], int, int]:
    ...
    if not registry_path.exists():
        errors.append("框架库: 缺少依赖登记表 registry.yaml")

    router = root / "references" / "README.md"
    if not router.exists():
        ...
        for phrase in ("竞争解释", "区分信号", "推翻条件", "待验证观点", "judgment-reasoning"):
    ...
    for path in files:
        ...
        if parse_error:
            errors.append(f"{display_rel(path, root)}: {parse_error}")
        metadata[path] = front
        asset_class = classify_asset(path, root, front)
        classes[path] = asset_class
        if asset_class == "invalid":
            errors.append(f"{display_rel(path, root)}: 文档位置或 document_type 不符合结构")
            continue
        if asset_class == "framework":
            for key in REQUIRED_FRONT_MATTER:
                if not front.get(key):
                    errors.append(f"{display_rel(path, root)}: 缺少 front matter 字段 {key}")
            if "builds_on" in front:
                errors.append(f"{display_rel(path, root)}: builds_on 已禁用，依赖只能写入 registry")
            framework_id = str(front.get("framework_id", ""))
            if framework_id:
                if framework_id in framework_ids:
                    errors.append(f"{display_rel(path, root)}: framework_id {framework_id} 重复")
            ...
            if front.get("document_type") != "semiconductor_scenario_card":
                errors.append(f"{display_rel(path, root)}: document_type 应为 semiconductor_scenario_card")
            if scenario_id:
                if scenario_id in scenario_ids:
                    errors.append(f"{display_rel(path, root)}: scenario_id {scenario_id} 重复")
    ...
    for framework_id, path in framework_ids.items():
        text = path.read_text(encoding="utf-8")
        front = metadata[path]
        relative = display_rel(path, root)
    ...
    link_files = [root / "SKILL.md", root / "references" / "README.md"] + files
    for path in link_files:
        ...
        for raw_target, resolved in local_markdown_links(path, text):
            if not resolved.exists():
                errors.append(f"{display_rel(path, root)}: 内部链接不存在 {raw_target}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="03_agent_capability/02_skills/research_design目录，默认使用脚本所在目录",
    )
